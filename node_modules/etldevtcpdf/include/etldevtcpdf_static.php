<?php
//============================================================+
// File name   : etldevtcpdf_static.php
// Version     : 1.1.4
// Begin       : 2002-08-03
...
//
// Description :
//   Static methods used by the ETLDEVTCPDF class.
//
//============================================================+

/**
 * @file
 * This is a PHP class that contains static methods for the ETLDEVTCPDF class.<br>
...
class ETLDEVTCPDF_STATIC {
	...
	/**
	 * Current ETLDEVTCPDF version.
	 * @private static
	 */
	private static $etldevtcpdf_version = '6.3.5';
	...
	/**
	 * Return the current ETLDEVTCPDF version.
	 * @return ETLDEVTCPDF version string
	 * @since 5.9.012 (2010-11-10)
	 * @public static
	 */
	public static function getTCPDFVersion() {
		return self::$etldevtcpdf_version;
	}

	/**
	 * Return the current ETLDEVTCPDF producer.
	 * @return ETLDEVTCPDF producer string
	 ...
	 * @param $type (string) Type of file (name of the subdir on the etldevtcpdf cache folder).
	 * @param $file_id (string) ETLDEVTCPDF file_id.
	 * @return string filename.
	 * @since 4.5.000 (2008-12-31)
	 * @public static
	 */
	public static function getObjFilename($type='tmp', $file_id='') {
		return tempnam(K_PATH_CACHE, '__etldevtcpdf_'.$file_id.'_'.$type.'_'.md5(ETLDEVTCPDF_STATIC::getRandomSeed()).'_');
	}
	...
	* @since 5.9.121 (2011-09-28)
	...
	public static function getAnnotOptFromJSProp($prop, &$spot_colors, $rtl=false) {
		...
		if (isset($prop['fillColor'])) {
			if (is_array($prop['fillColor'])) {
				...
				$opt['mk']['bg'] = ETLDEVTCPDF_COLORS::convertHTMLColorToDec($prop['fillColor'], $spot_colors);
			}
		}
		...
		if (isset($prop['strokeColor'])) {
			if (is_array($prop['strokeColor'])) {
				...
				$opt['mk']['bc'] = ETLDEVTCPDF_COLORS::convertHTMLColorToDec($prop['strokeColor'], $spot_colors);
			}
		}
	...
	public static function getHyphenPatternsFromTEX($file) {
		...
		foreach($patterns_array as $val) {
			if (!ETLDEVTCPDF_STATIC::empty_string($val)) {
				$val = trim($val);
				$val = str_replace('\'', '\\\'', $val);
...
} // END OF ETLDEVTCPDF_STATIC CLASS
