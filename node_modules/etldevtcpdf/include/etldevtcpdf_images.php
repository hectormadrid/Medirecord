<?php
//============================================================+
// File name   : etldevtcpdf_images.php
// Version     : 1.0.005
// Begin       : 2002-08-03
...
//
// Description :
//   Static image methods used by the ETLDEVTCPDF class.
//
//============================================================+

/**
 * @file
 * This is a PHP class that contains static image methods for the ETLDEVTCPDF class.<br>
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	...
	public static function getImageFileType($imgfile, $iminfo=array()) {
		...
		if (empty($type)) {
			...
			if (isset($fileinfo['extension']) AND (!ETLDEVTCPDF_STATIC::empty_string($fileinfo['extension']))) {
	...
	public static function _parsejpeg($file) {
		...
		while (($pos = strpos($data, "ICC_PROFILE\0", $offset)) !== false) {
			...
			$length = (ETLDEVTCPDF_STATIC::_getUSHORT($data, ($pos - 2)) - 16);
	...
	public static function _parsepng($file) {
		...
		$w = ETLDEVTCPDF_STATIC::_freadint($f);
		$h = ETLDEVTCPDF_STATIC::_freadint($f);
		$bpc = ord(fread($f, 1));
		$ct = ord(fread($f, 1));
		if ($ct == 0) {
			$colspace = 'DeviceGray';
		...
		$data = '';
		$icc = false;
		$n = ETLDEVTCPDF_STATIC::_freadint($f);
		do {
			$type = fread($f, 4);
			if ($type == 'PLTE') {
				// read palette
				$pal = ETLDEVTCPDF_STATIC::rfread($f, $n);
				fread($f, 4);
			} elseif ($type == 'tRNS') {
				// read transparency info
				$t = ETLDEVTCPDF_STATIC::rfread($f, $n);
				if ($ct == 0) { // DeviceGray
					$trns = array(ord($t[1]));
				...
				$data .= ETLDEVTCPDF_STATIC::rfread($f, $n);
				fread($f, 4);
			} elseif ($type == 'iCCP') {
				// skip profile name
				$len = 0;
				...
				$icc = ETLDEVTCPDF_STATIC::rfread($f, ($n - $len - 2));
				// decompress profile
				...
				ETLDEVTCPDF_STATIC::rfread($f, $n + 4);
			}
			$n = ETLDEVTCPDF_STATIC::_freadint($f);
		} while ($n);
...
} // END OF ETLDEVTCPDF_IMAGES CLASS
