<?php
//============================================================+
// File name   : etldevtcpdf_fonts.php
// Version     : 1.1.0
// Begin       : 2008-01-01
...
// -------------------------------------------------------------------
//
// Description :Font methods for ETLDEVTCPDF library.
//
//============================================================+

/**
 * @file
 * Unicode data and font methods for ETLDEVTCPDF library.
 * @author Nicola Asuni
 * @package com.tecnick.etldevtcpdf
 */

/**
 * @class ETLDEVTCPDF_FONTS
 * Font methods for ETLDEVTCPDF library.
 * @package com.tecnick.etldevtcpdf
 * @version 1.1.0
...
class ETLDEVTCPDF_FONTS {
	 ...
	 * @return (string) ETLDEVTCPDF font name or boolean false in case of error.
	 * @author Nicola Asuni
	 * @since 5.9.123 (2010-09-30)
	 * @public static
	 */
	public static function addTTFfont($fontfile, $fonttype='', $enc='', $flags=32, $outpath='', $platid=3, $encid=1, $addcbbox=false, $link=false) {
		if (!ETLDEVTCPDF_STATIC::file_exists($fontfile)) {
			// Could not find file
			return false;
		}
		// font metrics
		$fmetric = array();
		// build new font name for ETLDEVTCPDF compatibility
		...
		if (@ETLDEVTCPDF_STATIC::file_exists($outpath.$font_name.'.php')) {
		...
		if (empty($fonttype)) {
			if (ETLDEVTCPDF_STATIC::_getULONG($font, 0) == 0x10000) {
		...
		if (($fmetric['type'] == 'TrueType') OR ($fmetric['type'] == 'Type1')) {
			if (!empty($enc) AND ($enc != 'cp1252') AND isset(ETLDEVTCPDF_FONT_DATA::$encmap[$enc])) {
				// build differences from reference encoding
				$enc_ref = ETLDEVTCPDF_FONT_DATA::$encmap['cp1252'];
				$enc_target = ETLDEVTCPDF_FONT_DATA::$encmap[$enc];
				$last = 0;
				for ($i = 32; $i <= 255; ++$i) {
		...
		if ($fmetric['type'] == 'Type1') {
			...
			$fmetric['file'] .= '.z';
			$fp = ETLDEVTCPDF_STATIC::fopenLocal($outpath.$fmetric['file'], 'wb');
			...
			if (!empty($enc) AND isset(ETLDEVTCPDF_FONT_DATA::$encmap[$enc])) {
				$enc_map = ETLDEVTCPDF_FONT_DATA::$encmap[$enc];
			} else {
				$enc_map = false;
			}
			$fmetric['cw'] = '';
			...
			if (ETLDEVTCPDF_STATIC::_getULONG($font, $offset) != 0x10000) {
			...
			$numTables = ETLDEVTCPDF_STATIC::_getUSHORT($font, $offset);
			$offset += 2;
			...
			for ($i = 0; $i < $numTables; ++$i) {
				...
				$offset += 4;
				$table[$tag] = array();
				$table[$tag]['checkSum'] = ETLDEVTCPDF_STATIC::_getULONG($font, $offset);
				$offset += 4;
				$table[$tag]['offset'] = ETLDEVTCPDF_STATIC::_getULONG($font, $offset);
				$offset += 4;
				$table[$tag]['length'] = ETLDEVTCPDF_STATIC::_getULONG($font, $offset);
				$offset += 4;
			}
			// check magicNumber
			$offset = $table['head']['offset'] + 12;
			if (ETLDEVTCPDF_STATIC::_getULONG($font, $offset) != 0x5F0F3CF5) {
			...
			$offset += 2; // skip flags
			// get FUnits
			$fmetric['unitsPerEm'] = ETLDEVTCPDF_STATIC::_getUSHORT($font, $offset);
			$offset += 2;
			// units ratio constant
			$urk = (1000 / $fmetric['unitsPerEm']);
			$offset += 16; // skip created, modified
			$xMin = round(ETLDEVTCPDF_STATIC::_getFWORD($font, $offset) * $urk);
			$offset += 2;
			$yMin = round(ETLDEVTCPDF_STATIC::_getFWORD($font, $offset) * $urk);
			$offset += 2;
			$xMax = round(ETLDEVTCPDF_STATIC::_getFWORD($font, $offset) * $urk);
			$offset += 2;
			$yMax = round(ETLDEVTCPDF_STATIC::_getFWORD($font, $offset) * $urk);
			$offset += 2;
			$fmetric['bbox'] = ''.$xMin.' '.$yMin.' '.$xMax.' '.$yMax.'';
			$macStyle = ETLDEVTCPDF_STATIC::_getUSHORT($font, $offset);
			$offset += 2;
			// PDF font flags
			$fmetric['Flags'] = $flags;
			if (($macStyle & 2) == 2) {
			...
			$short_offset = (ETLDEVTCPDF_STATIC::_getSHORT($font, $offset) == 0);
			$offset += 2;
			...
			if ($short_offset) {
				...
				for ($i = 0; $i < $tot_num_glyphs; ++$i) {
					$indexToLoc[$i] = ETLDEVTCPDF_STATIC::_getUSHORT($font, $offset) * 2;
				...
				for ($i = 0; $i < $tot_num_glyphs; ++$i) {
					$indexToLoc[$i] = ETLDEVTCPDF_STATIC::_getULONG($font, $offset);
					if (isset($indexToLoc[($i - 1)]) && ($indexToLoc[$i] == $indexToLoc[($i - 1)])) {
						// the last glyph didn't have an outline
						unset($indexToLoc[($i - 1)]);
					}
			...
			$numEncodingTables = ETLDEVTCPDF_STATIC::_getUSHORT($font, $offset);
			$offset += 2;
			$encodingTables = array();
			for ($i = 0; $i < $numEncodingTables; ++$i) {
				$encodingTables[$i]['platformID'] = ETLDEVTCPDF_STATIC::_getUSHORT($font, $offset);
				$offset += 2;
				$encodingTables[$i]['encodingID'] = ETLDEVTCPDF_STATIC::_getUSHORT($font, $offset);
				$offset += 2;
				$encodingTables[$i]['offset'] = ETLDEVTCPDF_STATIC::_getULONG($font, $offset);
				$offset += 4;
			}
			// ---------- get os/2 metrics ----------
			...
			// xAvgCharWidth
			$fmetric['AvgWidth'] = round(ETLDEVTCPDF_STATIC::_getFWORD($font, $offset) * $urk);
			$offset += 2;
			// usWeightClass
			$usWeightClass = round(ETLDEVTCPDF_STATIC::_getUFWORD($font, $offset) * $urk);
			...
			$fsType = ETLDEVTCPDF_STATIC::_getSHORT($font, $offset);
			$offset += 2;
			if ($fsType == 2) {
			...
			$numNameRecords = ETLDEVTCPDF_STATIC::_getUSHORT($font, $offset);
			$offset += 2;
			// Offset to start of string storage (from start of table).
			$stringStorageOffset = ETLDEVTCPDF_STATIC::_getUSHORT($font, $offset);
			$offset += 2;
			for ($i = 0; $i < $numNameRecords; ++$i) {
				$offset += 6; // skip Platform ID, Platform-specific encoding ID, Language ID.
				// Name ID.
				$nameID = ETLDEVTCPDF_STATIC::_getUSHORT($font, $offset);
				$offset += 2;
				if ($nameID == 6) {
					// String length (in bytes).
					$stringLength = ETLDEVTCPDF_STATIC::_getUSHORT($font, $offset);
					$offset += 2;
					// String offset from start of storage area (in bytes).
					$stringOffset = ETLDEVTCPDF_STATIC::_getUSHORT($font, $offset);
					$offset += 2;
			...
			$fmetric['italicAngle'] = ETLDEVTCPDF_STATIC::_getFIXED($font, $offset);
			$offset += 4;
			$fmetric['underlinePosition'] = round(ETLDEVTCPDF_STATIC::_getFWORD($font, $offset) * $urk);
			$offset += 2;
			$fmetric['underlineThickness'] = round(ETLDEVTCPDF_STATIC::_getFWORD($font, $offset) * $urk);
			$offset += 2;
			$isFixedPitch = (ETLDEVTCPDF_STATIC::_getULONG($font, $offset) == 0) ? false : true;
			$offset += 2;
			if ($isFixedPitch) {
				$fmetric['Flags'] |= 1;
			}
			...
			// Ascender
			$fmetric['Ascent'] = round(ETLDEVTCPDF_STATIC::_getFWORD($font, $offset) * $urk);
			$offset += 2;
			// Descender
			$fmetric['Descent'] = round(ETLDEVTCPDF_STATIC::_getFWORD($font, $offset) * $urk);
			$offset += 2;
			// LineGap
			$fmetric['Leading'] = round(ETLDEVTCPDF_STATIC::_getFWORD($font, $offset) * $urk);
			$offset += 2;
			// advanceWidthMax
			$fmetric['MaxWidth'] = round(ETLDEVTCPDF_STATIC::_getUFWORD($font, $offset) * $urk);
			$offset += 2;
			$offset += 22; // skip some values
			// get the number of hMetric entries in hmtx table
			$numberOfHMetrics = ETLDEVTCPDF_STATIC::_getUSHORT($font, $offset);
			// ---------- get maxp data ----------
			$offset = $table['maxp']['offset'];
			$offset += 4; // skip Table version number
			// get the the number of glyphs in the font.
			$numGlyphs = ETLDEVTCPDF_STATIC::_getUSHORT($font, $offset);
			// ---------- get CIDToGIDMap ----------
			$ctg = array();
			foreach ($encodingTables as $enctable) {
				...
				if (($enctable['platformID'] == $platid) AND ($enctable['encodingID'] == $encid)) {
					...
					$format = ETLDEVTCPDF_STATIC::_getUSHORT($font, $offset);
					$offset += 2;
					switch ($format) {
						case 0: { // Format 0: Byte encoding table
							$offset += 4; // skip length and version/language
							for ($c = 0; $c < 256; ++$c) {
								$g = ETLDEVTCPDF_STATIC::_getBYTE($font, $offset);
								$ctg[$c] = $g;
								++$offset;
							}
							break;
						...
						case 2: { // Format 2: High-byte mapping through table
							...
							for ($i = 0; $i < 256; ++$i) {
								...
								$subHeaderKeys[$i] = (ETLDEVTCPDF_STATIC::_getUSHORT($font, $offset) / 8);
								$offset += 2;
							...
							for ($k = 0; $k < $numSubHeaders; ++$k) {
								$subHeaders[$k]['firstCode'] = ETLDEVTCPDF_STATIC::_getUSHORT($font, $offset);
								$offset += 2;
								$subHeaders[$k]['entryCount'] = ETLDEVTCPDF_STATIC::_getUSHORT($font, $offset);
								$offset += 2;
								$subHeaders[$k]['idDelta'] = ETLDEVTCPDF_STATIC::_getUSHORT($font, $offset);
								$offset += 2;
								$subHeaders[$k]['idRangeOffset'] = ETLDEVTCPDF_STATIC::_getUSHORT($font, $offset);
								$offset += 2;
							...
							for ($k = 0; $k < $numGlyphIndexArray; ++$k) {
								$glyphIndexArray[$k] = ETLDEVTCPDF_STATIC::_getUSHORT($font, $offset);
								$offset += 2;
							}
							for ($i = 0; $i < 256; ++$i) {
								$k = $subHeaderKeys[$i];
						...
						case 6: { // Format 6: Trimmed table mapping
							$offset += 4; // skip length and version/language
							$firstCode = ETLDEVTCPDF_STATIC::_getUSHORT($font, $offset);
							$offset += 2;
							$entryCount = ETLDEVTCPDF_STATIC::_getUSHORT($font, $offset);
							$offset += 2;
							for ($k = 0; $k < $entryCount; ++$k) {
								$c = ($k + $firstCode);
								$g = ETLDEVTCPDF_STATIC::_getUSHORT($font, $offset);
								$offset += 2;
								$ctg[$c] = $g;
							}
							break;
						...
						case 8: { // Format 8: Mixed 16-bit and 32-bit coverage
							...
							for ($k = 0; $k < 8192; ++$k) {
								$is32[$k] = ETLDEVTCPDF_STATIC::_getBYTE($font, $offset);
								++$offset;
							}
							$nGroups = ETLDEVTCPDF_STATIC::_getULONG($font, $offset);
							$offset += 4;
							for ($i = 0; $i < $nGroups; ++$i) {
								$startCharCode = ETLDEVTCPDF_STATIC::_getULONG($font, $offset);
								$offset += 4;
								$endCharCode = ETLDEVTCPDF_STATIC::_getULONG($font, $offset);
								$offset += 4;
								$startGlyphID = ETLDEVTCPDF_STATIC::_getULONG($font, $offset);
								$offset += 4;
						...
						case 10: { // Format 10: Trimmed array
							...
							$startCharCode = ETLDEVTCPDF_STATIC::_getULONG($font, $offset);
							$offset += 4;
							$numChars = ETLDEVTCPDF_STATIC::_getULONG($font, $offset);
							$offset += 4;
						...
						case 12: { // Format 12: Segmented coverage
							...
							for ($k = 0; $k < $nGroups; ++$k) {
								...
								$endCharCode = ETLDEVTCPDF_STATIC::_getULONG($font, $offset);
								$offset += 4;
								$startGlyphCode = ETLDEVTCPDF_STATIC::_getULONG($font, $offset);
								$offset += 4;
			...
			$yMin = ETLDEVTCPDF_STATIC::_getFWORD($font, $offset);
			$offset += 4;
			$yMax = ETLDEVTCPDF_STATIC::_getFWORD($font, $offset);
			$offset += 2;
			...
			for ($i = 0 ; $i < $numberOfHMetrics; ++$i) {
				$cw[$i] = round(ETLDEVTCPDF_STATIC::_getUFWORD($font, $offset) * $urk);
				$offset += 4; // skip lsb
			}
			...
			for ($cid = 0; $cid <= 65535; ++$cid) {
				if (isset($ctg[$cid])) {
					...
					if ($addcbbox AND isset($indexToLoc[$ctg[$cid]])) {
						...
						$xMin = round(ETLDEVTCPDF_STATIC::_getFWORD($font, $offset + 2) * $urk);
						$yMin = round(ETLDEVTCPDF_STATIC::_getFWORD($font, $offset + 4) * $urk);
						$xMax = round(ETLDEVTCPDF_STATIC::_getFWORD($font, $offset + 6) * $urk);
						$yMax = round(ETLDEVTCPDF_STATIC::_getFWORD($font, $offset + 8) * $urk);
		...
		$pfile = '<'.'?'.'php'."\n";
		$pfile .= '// ETLDEVTCPDF FONT FILE DESCRIPTION'."\n";
		...
		if ($fmetric['type'] == 'Type1') {
			...
			if ($fmetric['type'] == 'cidfont0') {
				...
				$fp = ETLDEVTCPDF_STATIC::fopenLocal($outpath.$fmetric['ctg'], 'wb');
		...
		$pfile .= '// --- EOF ---'."\n";
		// store file
		$fp = ETLDEVTCPDF_STATIC::fopenLocal($outpath.$font_name.'.php', 'w');
		fwrite($fp, $pfile);
		fclose($fp);
		// return ETLDEVTCPDF font name
		return $font_name;
	}
		...
		while (!empty($new_sga)) {
			...
			foreach ($sga as $key => $val) {
				if (isset($indexToLoc[$key])) {
					...
					$numberOfContours = ETLDEVTCPDF_STATIC::_getSHORT($font, $offset);
					$offset += 2;
					if ($numberOfContours < 0) { // composite glyph
						$offset += 8; // skip xMin, yMin, xMax, yMax
						do {
							$flags = ETLDEVTCPDF_STATIC::_getUSHORT($font, $offset);
							$offset += 2;
							$glyphIndex = ETLDEVTCPDF_STATIC::_getUSHORT($font, $offset);
							$offset += 2;
	...
	public static function getFontFullPath($file, $fontdir=false) {
		...
		if (($fontdir !== false) AND @ETLDEVTCPDF_STATIC::file_exists($fontdir.$file)) {
			$fontfile = $fontdir.$file;
		} elseif (@ETLDEVTCPDF_STATIC::file_exists(self::_getfontpath().$file)) {
			$fontfile = self::_getfontpath().$file;
		} elseif (@ETLDEVTCPDF_STATIC::file_exists($file)) {
			$fontfile = $file;
		}
		return $fontfile;
	}
...
// ====================================================================================================================
	...
	public static function UTF8ArrayToUniArray($ta, $isunicode=true) {
		if ($isunicode) {
			return array_map(array('ETLDEVTCPDF_FONTS', 'unichrUnicode'), $ta);
		}
		return array_map(array('ETLDEVTCPDF_FONTS', 'unichrASCII'), $ta);
	}
	...
	public static function UTF8ArrToLatin1Arr($unicode) {
		...
		foreach ($unicode as $char) {
			if ($char < 256) {
				$outarr[] = $char;
			} elseif (array_key_exists($char, ETLDEVTCPDF_FONT_DATA::$uni_utf8tolatin)) {
				// map from UTF-8
				$outarr[] = ETLDEVTCPDF_FONT_DATA::$uni_utf8tolatin[$char];
			} elseif ($char == 0xFFFD) {
				// skip
			} else {
				$outarr[] = 63; // '?' character
	...
	public static function UTF8ArrToLatin1($unicode) {
		...
		foreach ($unicode as $char) {
			if ($char < 256) {
				...
				// map from UTF-8
				$outstr .= chr(ETLDEVTCPDF_FONT_DATA::$uni_utf8tolatin[$char]);
			} elseif ($char == 0xFFFD) {
				// skip
			} else {
				$outstr .= '?';
	...
	public static function UTF8StringToArray($str, $isunicode=true, &$currentfont) {
		if ($isunicode) {
			...
			$chars = ETLDEVTCPDF_STATIC::pregSplit('//','u', $str, -1, PREG_SPLIT_NO_EMPTY);
			$carr = array_map(array('ETLDEVTCPDF_FONTS', 'uniord'), $chars);
		} else {
			$chars = str_split($str);
	...
	public static function utf8Bidi($ta, $str='', $forcertl=false, $isunicode=true, &$currentfont) {
		...
		// max level
		$maxlevel = 0;
		if (ETLDEVTCPDF_STATIC::empty_string($str)) {
			// create string from array
			$str = self::UTF8ArrSubString($ta, '', '', $isunicode);
		}
		// check if string contains arabic text
		if (preg_match(ETLDEVTCPDF_FONT_DATA::$uni_RE_PATTERN_ARABIC, $str)) {
			$arabic = true;
		} else {
			$arabic = false;
		}
		// check if string contains RTL text
		if (!($forcertl OR $arabic OR preg_match(ETLDEVTCPDF_FONT_DATA::$uni_RE_PATTERN_RTL, $str))) {
			return $ta;
		}
		...
		if ($forcertl == 'R') {
			...
			for ($i=0; $i < $numchars; ++$i) {
				$type = ETLDEVTCPDF_FONT_DATA::$uni_type[$ta[$i]];
				if ($type == 'L') {
					$pel = 0;
					break;
		...
		for ($i=0; $i < $numchars; ++$i) {
			if ($ta[$i] == ETLDEVTCPDF_FONT_DATA::$uni_RLE) {
				// X2. With each RLE, compute the least greater odd embedding level.
				//	a. If this new level would be valid, then this embedding code is valid. Remember (push) the current embedding level and override status. Reset the current level to this new level, and reset the override status to neutral.
				//	b. If the new level would not be valid, then this code is invalid. Do not change the current level or override status.
				$next_level = $cel + ($cel % 2) + 1;
				if ($next_level < 62) {
					$remember[] = array('num' => ETLDEVTCPDF_FONT_DATA::$uni_RLE, 'cel' => $cel, 'dos' => $dos);
					$cel = $next_level;
					$dos = 'N';
					$sor = $eor;
					$eor = $cel % 2 ? 'R' : 'L';
				}
			} elseif ($ta[$i] == ETLDEVTCPDF_FONT_DATA::$uni_LRE) {
				// X3. With each LRE, compute the least greater even embedding level.
				//	a. If this new level would be valid, then this embedding code is valid. Remember (push) the current embedding level and override status. Reset the current level to this new level, and reset the override status to neutral.
				//	b. If the new level would not be valid, then this code is invalid. Do not change the current level or override status.
				$next_level = $cel + 2 - ($cel % 2);
				if ( $next_level < 62 ) {
					$remember[] = array('num' => ETLDEVTCPDF_FONT_DATA::$uni_LRE, 'cel' => $cel, 'dos' => $dos);
					$cel = $next_level;
					$dos = 'N';
					$sor = $eor;
					$eor = $cel % 2 ? 'R' : 'L';
				}
			} elseif ($ta[$i] == ETLDEVTCPDF_FONT_DATA::$uni_RLO) {
				// X4. With each RLO, compute the least greater odd embedding level.
				//	a. If this new level would be valid, then this embedding code is valid. Remember (push) the current embedding level and override status. Reset the current level to this new level, and reset the override status to right-to-left.
				//	b. If the new level would not be valid, then this code is invalid. Do not change the current level or override status.
				$next_level = $cel + ($cel % 2) + 1;
				if ($next_level < 62) {
					$remember[] = array('num' => ETLDEVTCPDF_FONT_DATA::$uni_RLO, 'cel' => $cel, 'dos' => $dos);
					$cel = $next_level;
					$dos = 'R';
					$sor = $eor;
					$eor = $cel % 2 ? 'R' : 'L';
				}
			} elseif ($ta[$i] == ETLDEVTCPDF_FONT_DATA::$uni_LRO) {
				// X5. With each LRO, compute the least greater even embedding level.
				//	a. If this new level would be valid, then this embedding code is valid. Remember (push) the current embedding level and override status. Reset the current level to this new level, and reset the override status to left-to-right.
				//	b. If the new level would not be valid, then this code is invalid. Do not change the current level or override status.
				$next_level = $cel + 2 - ($cel % 2);
				if ( $next_level < 62 ) {
					$remember[] = array('num' => ETLDEVTCPDF_FONT_DATA::$uni_LRO, 'cel' => $cel, 'dos' => $dos);
					$cel = $next_level;
					$dos = 'L';
					$sor = $eor;
					$eor = $cel % 2 ? 'R' : 'L';
				}
			} elseif ($ta[$i] == ETLDEVTCPDF_FONT_DATA::$uni_PDF) {
				// X7. With each PDF, determine the matching embedding or override code. If there was a valid matching code, restore (pop) the last remembered (pushed) embedding level and directional override.
				if (count($remember)) {
					$last = count($remember ) - 1;
					if (($remember[$last]['num'] == ETLDEVTCPDF_FONT_DATA::$uni_RLE) OR
						($remember[$last]['num'] == ETLDEVTCPDF_FONT_DATA::$uni_LRE) OR
						($remember[$last]['num'] == ETLDEVTCPDF_FONT_DATA::$uni_RLO) OR
						($remember[$last]['num'] == ETLDEVTCPDF_FONT_DATA::$uni_LRO)) {
			...
			} elseif (($ta[$i] != ETLDEVTCPDF_FONT_DATA::$uni_RLE) AND
							 ($ta[$i] != ETLDEVTCPDF_FONT_DATA::$uni_LRE) AND
							 ($ta[$i] != ETLDEVTCPDF_FONT_DATA::$uni_RLO) AND
							 ($ta[$i] != ETLDEVTCPDF_FONT_DATA::$uni_LRO) AND
							 ($ta[$i] != ETLDEVTCPDF_FONT_DATA::$uni_PDF)) {
				...
				if ($dos != 'N') {
					...
					if (isset(ETLDEVTCPDF_FONT_DATA::$uni_type[$ta[$i]])) {
						$chardir = ETLDEVTCPDF_FONT_DATA::$uni_type[$ta[$i]];
					} else {
						$chardir = 'L';
					}
				}
		...
		if ($arabic) {
			...
			$x = 0;
			for ($i=0; $i < $numchars; ++$i) {
				if ((ETLDEVTCPDF_FONT_DATA::$uni_type[$chardata[$i]['char']] == 'AL') OR ($chardata[$i]['char'] == 32) OR ($chardata[$i]['char'] == 8204)) {
					$charAL[$x] = $chardata[$i];
					$charAL[$x]['i'] = $i;
					$chardata[$i]['x'] = $x;
					++$x;
			...
			for ($i=0; $i < $numchars; ++$i) {
				...
				if (ETLDEVTCPDF_FONT_DATA::$uni_type[$thischar['char']] == 'AL') {
					$x = $thischar['x'];
					if ($x > 0) {
						$prevchar = $charAL[($x-1)];
					} else {
					...
					if (($prevchar !== false) AND ($prevchar['char'] == 1604) AND (in_array($thischar['char'], $alfletter))) {
						$arabicarr = ETLDEVTCPDF_FONT_DATA::$uni_laa_array;
						$laaletter = true;
						if ($x > 1) {
							$prevchar = $charAL[($x-2)];
						} else {
						...
						$arabicarr = ETLDEVTCPDF_FONT_DATA::$uni_arabicsubst;
						$laaletter = false;
					}
					if (($prevchar !== false) AND ($nextchar !== false) AND
						((ETLDEVTCPDF_FONT_DATA::$uni_type[$prevchar['char']] == 'AL') OR (ETLDEVTCPDF_FONT_DATA::$uni_type[$prevchar['char']] == 'NSM')) AND
						((ETLDEVTCPDF_FONT_DATA::$uni_type[$nextchar['char']] == 'AL') OR (ETLDEVTCPDF_FONT_DATA::$uni_type[$nextchar['char']] == 'NSM')) AND
			...
			for ($i = 0; $i < ($numchars-1); ++$i) {
				if (($chardata2[$i]['char'] == 1617) AND (isset(ETLDEVTCPDF_FONT_DATA::$uni_diacritics[($chardata2[$i+1]['char'])]))) {
					// check if the subtitution font is defined on current font
					if (isset($currentfont['cw'][(ETLDEVTCPDF_FONT_DATA::$uni_diacritics[($chardata2[$i+1]['char'])])])) {
						$chardata2[$i]['char'] = false;
						$chardata2[$i+1]['char'] = ETLDEVTCPDF_FONT_DATA::$uni_diacritics[($chardata2[$i+1]['char'])];
					}
				}
			}
		...
		for ($j=$maxlevel; $j > 0; $j--) {
			...
			for ($i=0; $i < $numchars; ++$i) {
				if ($chardata[$i]['level'] >= $j) {
					$onlevel = true;
					if (isset(ETLDEVTCPDF_FONT_DATA::$uni_mirror[$chardata[$i]['char']])) {
						// L4. A character is depicted by a mirrored glyph if and only if (a) the resolved directionality of that character is R, and (b) the Bidi_Mirrored property value of that character is true.
						$chardata[$i]['char'] = ETLDEVTCPDF_FONT_DATA::$uni_mirror[$chardata[$i]['char']];
					}
					$revarr[] = $chardata[$i];
				} else {
					if ($onlevel) {
...
} // END OF ETLDEVTCPDF_FONTS CLASS
