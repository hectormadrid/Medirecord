 ...
 * ETLDEVTCPDF project (http://www.etldevtcpdf.org) was originally derived in 2002 from the Public Domain FPDF class by Olivier Plathey (http://www.fpdf.org), but now is almost entirely rewritten.<br>
 * <h3>ETLDEVTCPDF main features are:</h3>
 * <ul>
...
// ETLDEVTCPDF configuration
require_once(dirname(__FILE__).'/etldevtcpdf_autoconfig.php');
// ETLDEVTCPDF static font methods and data
require_once(dirname(__FILE__).'/include/etldevtcpdf_font_data.php');
// ETLDEVTCPDF static font methods and data
require_once(dirname(__FILE__).'/include/etldevtcpdf_fonts.php');
// ETLDEVTCPDF static color methods and data
require_once(dirname(__FILE__).'/include/etldevtcpdf_colors.php');
// ETLDEVTCPDF static image methods and data
require_once(dirname(__FILE__).'/include/etldevtcpdf_images.php');
// ETLDEVTCPDF static methods and data
require_once(dirname(__FILE__).'/include/etldevtcpdf_static.php');
 ...
 * ETLDEVTCPDF project (http://www.etldevtcpdf.org) has been originally derived in 2002 from the Public Domain FPDF class by Olivier Plathey (http://www.fpdf.org), but now is almost entirely rewritten.<br>
 * @package com.tecnick.etldevtcpdf
...
class ETLDEVTCPDF {
    ...
    /**
     * Current index_protocol number.
     * @protected
     */
    Public $index_protocol;
	...
	/**
	 * If true print ETLDEVTCPDF meta link.
	 * @protected
	 * @since 5.9.152 (2012-03-23)
	 */
	 ...
	 *
	 * IMPORTANT: Please note that this method sets the mb_internal_encoding to ASCII, so if you are using the mbstring module functions with ETLDEVTCPDF you need to correctly set/unset the mb_internal_encoding when needed.
	 *
	 ...
	 */
	public function __construct($orientation='P', $unit='mm', $format='A4', $unicode=true, $encoding='UTF-8', $diskcache=false, $pdfa=false, $index_protocol=null) {
		...
		$this->file_id = md5(ETLDEVTCPDF_STATIC::getRandomSeed('ETLDEVTCPDF'.$orientation.$unit.$serformat.$encoding));
		...
		ETLDEVTCPDF_FONTS::utf8Bidi(array(), '', false, $this->isunicode, $this->CurrentFont);
		// set default font
	...
	protected function setPageFormat($format, $orientation='P') {
		...
		if (is_string($format)) {
			...
			$pf = ETLDEVTCPDF_STATIC::getPageSizeFromFormat($format);
			$this->fwPt = $pf[0];
			$this->fhPt = $pf[1];
		} else {
			// the boundaries of the physical medium on which the page shall be displayed or printed
			if (isset($format['MediaBox'])) {
				$this->pagedim = ETLDEVTCPDF_STATIC::setPageBoxes($this->page, 'MediaBox', $format['MediaBox']['llx'], $format['MediaBox']['lly'], $format['MediaBox']['urx'], $format['MediaBox']['ury'], false, $this->k, $this->pagedim);
				...
				if (isset($format[0]) AND is_numeric($format[0]) AND isset($format[1]) AND is_numeric($format[1])) {
					...
					$pf = ETLDEVTCPDF_STATIC::getPageSizeFromFormat($format['format']);
				}
				$this->fwPt = $pf[0];
				$this->fhPt = $pf[1];
				$this->pagedim = ETLDEVTCPDF_STATIC::setPageBoxes($this->page, 'MediaBox', 0, 0, $this->fwPt, $this->fhPt, true, $this->k, $this->pagedim);
			}
			// the visible region of default user space
			if (isset($format['CropBox'])) {
				$this->pagedim = ETLDEVTCPDF_STATIC::setPageBoxes($this->page, 'CropBox', $format['CropBox']['llx'], $format['CropBox']['lly'], $format['CropBox']['urx'], $format['CropBox']['ury'], false, $this->k, $this->pagedim);
			}
			// the region to which the contents of the page shall be clipped when output in a production environment
			if (isset($format['BleedBox'])) {
				$this->pagedim = ETLDEVTCPDF_STATIC::setPageBoxes($this->page, 'BleedBox', $format['BleedBox']['llx'], $format['BleedBox']['lly'], $format['BleedBox']['urx'], $format['BleedBox']['ury'], false, $this->k, $this->pagedim);
			}
			// the intended dimensions of the finished page after trimming
			if (isset($format['TrimBox'])) {
				$this->pagedim = ETLDEVTCPDF_STATIC::setPageBoxes($this->page, 'TrimBox', $format['TrimBox']['llx'], $format['TrimBox']['lly'], $format['TrimBox']['urx'], $format['TrimBox']['ury'], false, $this->k, $this->pagedim);
			}
			// the page's meaningful content (including potential white space)
			if (isset($format['ArtBox'])) {
				$this->pagedim = ETLDEVTCPDF_STATIC::setPageBoxes($this->page, 'ArtBox', $format['ArtBox']['llx'], $format['ArtBox']['lly'], $format['ArtBox']['urx'], $format['ArtBox']['ury'], false, $this->k, $this->pagedim);
			}
	...
	public function setPageOrientation($orientation, $autopagebreak='', $bottommargin='') {
		...
		if (!isset($this->pagedim[$this->page]['CropBox'])) {
			// the visible region of default user space
			$this->pagedim = ETLDEVTCPDF_STATIC::setPageBoxes($this->page, 'CropBox', $this->pagedim[$this->page]['MediaBox']['llx'], $this->pagedim[$this->page]['MediaBox']['lly'], $this->pagedim[$this->page]['MediaBox']['urx'], $this->pagedim[$this->page]['MediaBox']['ury'], true, $this->k, $this->pagedim);
		}
		if (!isset($this->pagedim[$this->page]['BleedBox'])) {
			// the region to which the contents of the page shall be clipped when output in a production environment
			$this->pagedim = ETLDEVTCPDF_STATIC::setPageBoxes($this->page, 'BleedBox', $this->pagedim[$this->page]['CropBox']['llx'], $this->pagedim[$this->page]['CropBox']['lly'], $this->pagedim[$this->page]['CropBox']['urx'], $this->pagedim[$this->page]['CropBox']['ury'], true, $this->k, $this->pagedim);
		}
		if (!isset($this->pagedim[$this->page]['TrimBox'])) {
			// the intended dimensions of the finished page after trimming
			$this->pagedim = ETLDEVTCPDF_STATIC::setPageBoxes($this->page, 'TrimBox', $this->pagedim[$this->page]['CropBox']['llx'], $this->pagedim[$this->page]['CropBox']['lly'], $this->pagedim[$this->page]['CropBox']['urx'], $this->pagedim[$this->page]['CropBox']['ury'], true, $this->k, $this->pagedim);
		}
		if (!isset($this->pagedim[$this->page]['ArtBox'])) {
			// the page's meaningful content (including potential white space)
			$this->pagedim = ETLDEVTCPDF_STATIC::setPageBoxes($this->page, 'ArtBox', $this->pagedim[$this->page]['CropBox']['llx'], $this->pagedim[$this->page]['CropBox']['lly'], $this->pagedim[$this->page]['CropBox']['urx'], $this->pagedim[$this->page]['CropBox']['ury'], true, $this->k, $this->pagedim);
		}
		...
		if ((abs($this->pagedim[$this->page]['MediaBox']['urx'] - $this->hPt) < $this->feps) AND (abs($this->pagedim[$this->page]['MediaBox']['ury'] - $this->wPt) < $this->feps)){
			...
			$this->pagedim = ETLDEVTCPDF_STATIC::swapPageBoxCoordinates($this->page, $this->pagedim);
		}
		$this->w = ($this->wPt / $this->k);
		$this->h = ($this->hPt / $this->k);
		if (ETLDEVTCPDF_STATIC::empty_string($autopagebreak)) {
		...
		if (ETLDEVTCPDF_STATIC::empty_string($bottommargin)) {
	...
	public function SetDisplayMode($zoom, $layout='SinglePage', $mode='UseNone') {
		...
		$this->LayoutMode = ETLDEVTCPDF_STATIC::getPageLayoutMode($layout);
		$this->PageMode = ETLDEVTCPDF_STATIC::getPageMode($mode);
	}
	...
	/**
	 * Throw an exception or print an error message and die if the K_ETLDEVTCPDF_PARSER_THROW_EXCEPTION_ERROR constant is set to true.
	...
	public function Error($msg) {
		...
		$this->_destroy(true);
		if (defined('K_ETLDEVTCPDF_THROW_EXCEPTION_ERROR') AND !K_ETLDEVTCPDF_THROW_EXCEPTION_ERROR) {
			die('<strong>ETLDEVTCPDF ERROR: </strong>'.$msg);
		} else {
			throw new Exception('ETLDEVTCPDF ERROR: '.$msg);
		}
	}
	...
	public function Header() {
		if ($this->header_xobjid === false) {
			...
			if (($headerdata['logo']) AND ($headerdata['logo'] != K_BLANK_IMAGE)) {
				$imgtype = ETLDEVTCPDF_IMAGES::getImageFileType(K_PATH_IMAGES.$headerdata['logo']);
	...
	protected function setTableHeader() {
		...
		if (!ETLDEVTCPDF_STATIC::empty_string($this->thead) AND (!$this->inthead)) {
			// set margins
	...
	public function setSpotColor($type, $name, $tint=100) {
		$spotcolor = ETLDEVTCPDF_COLORS::getSpotColor($name, $this->spot_colors);
		if ($spotcolor === false) {
	...
	public function GetStringWidth($s, $fontname='', $fontstyle='', $fontsize=0, $getarray=false) {
		return $this->GetArrStringWidth(ETLDEVTCPDF_FONTS::utf8Bidi(ETLDEVTCPDF_FONTS::UTF8StringToArray($s, $this->isunicode, $this->CurrentFont), $s, $this->tmprtl, $this->isunicode, $this->CurrentFont), $fontname, $fontstyle, $fontsize, $getarray);
	}
	...
	public function GetArrStringWidth($sa, $fontname='', $fontstyle='', $fontsize=0, $getarray=false) {
		...
		if (!ETLDEVTCPDF_STATIC::empty_string($fontname)) {
		...
		if ($this->isunicode AND (!$this->isUnicodeFont())) {
			$sa = ETLDEVTCPDF_FONTS::UTF8ArrToLatin1Arr($sa);
		}
		$w = 0; // total width
	...
	public function GetNumChars($s) {
		if ($this->isUnicodeFont()) {
			return count(ETLDEVTCPDF_FONTS::UTF8StringToArray($s, $this->isunicode, $this->CurrentFont));
		}
		return strlen($s);
	}
	...
	protected function getFontsList() {
		if (($fontsdir = opendir(ETLDEVTCPDF_FONTS::_getfontpath())) !== false) {
	...
	public function AddFont($family, $style='', $fontfile='', $subset='default') {
		...
		if (ETLDEVTCPDF_STATIC::empty_string($family)) {
			if (!ETLDEVTCPDF_STATIC::empty_string($this->FontFamily)) {
		...
		$fontdir = false;
		if (!ETLDEVTCPDF_STATIC::empty_string($fontfile)) {
			$fontdir = dirname($fontfile);
			if (ETLDEVTCPDF_STATIC::empty_string($fontdir) OR ($fontdir == '.')) {
				$fontdir = '';
			} else {
				$fontdir .= '/';
			}
		...
		if (ETLDEVTCPDF_STATIC::empty_string($fontfile) OR (!@ETLDEVTCPDF_STATIC::file_exists($fontfile))) {
			// build a standard filenames for specified font
			$tmp_fontfile = str_replace(' ', '', $family).strtolower($style).'.php';
			$fontfile = ETLDEVTCPDF_FONTS::getFontFullPath($tmp_fontfile, $fontdir);
			if (ETLDEVTCPDF_STATIC::empty_string($fontfile)) {
				$missing_style = true;
		...
		// include font file
		if (!ETLDEVTCPDF_STATIC::empty_string($fontfile) AND (@ETLDEVTCPDF_STATIC::file_exists($fontfile))) {
			include($fontfile);
		} else {
		...
		if (!isset($file) OR ETLDEVTCPDF_STATIC::empty_string($file)) {
			$file = '';
		}
		if (!isset($enc) OR ETLDEVTCPDF_STATIC::empty_string($enc)) {
			$enc = '';
		}
		if (!isset($cidinfo) OR ETLDEVTCPDF_STATIC::empty_string($cidinfo)) {
			$cidinfo = array('Registry'=>'Adobe', 'Ordering'=>'Identity', 'Supplement'=>0);
			$cidinfo['uni2cid'] = array();
		}
		if (!isset($ctg) OR ETLDEVTCPDF_STATIC::empty_string($ctg)) {
			$ctg = '';
		}
		if (!isset($desc) OR ETLDEVTCPDF_STATIC::empty_string($desc)) {
			$desc = array();
		}
		if (!isset($up) OR ETLDEVTCPDF_STATIC::empty_string($up)) {
			$up = -100;
		}
		if (!isset($ut) OR ETLDEVTCPDF_STATIC::empty_string($ut)) {
			$ut = 50;
		}
		if (!isset($cw) OR ETLDEVTCPDF_STATIC::empty_string($cw)) {
			$cw = array();
		}
		if (!isset($dw) OR ETLDEVTCPDF_STATIC::empty_string($dw)) {
			// set default width
		...
		if (!ETLDEVTCPDF_STATIC::empty_string($file)) {
	...
	public function isCharDefined($char, $font='', $style='') {
		if (is_string($char)) {
			// get character code
			$char = ETLDEVTCPDF_FONTS::UTF8StringToArray($char, $this->isunicode, $this->CurrentFont);
			$char = $char[0];
		}
		if (ETLDEVTCPDF_STATIC::empty_string($font)) {
			if (ETLDEVTCPDF_STATIC::empty_string($style)) {
	...
	public function replaceMissingChars($text, $font='', $style='', $subs=array()) {
		...
		$fontdata = $this->AddFont($font, $style);
		$fontinfo = $this->getFontBuffer($fontdata['fontkey']);
		$uniarr = ETLDEVTCPDF_FONTS::UTF8StringToArray($text, $this->isunicode, $this->CurrentFont);
		...
		return ETLDEVTCPDF_FONTS::UniArrSubString(ETLDEVTCPDF_FONTS::UTF8ArrayToUniArray($uniarr, $this->isunicode));
	}
	...
	public function Annotation($x, $y, $w, $h, $text, $opt=array('Subtype'=>'Text'), $spaces=0) {
		...
		if (!$this->pdfa_mode || ($this->pdfa_mode && $this->pdfa_version == 3)) {
			if ((($opt['Subtype'] == 'FileAttachment') OR ($opt['Subtype'] == 'Sound')) AND (!ETLDEVTCPDF_STATIC::empty_string($opt['FS']))
				AND (@ETLDEVTCPDF_STATIC::file_exists($opt['FS']) OR ETLDEVTCPDF_STATIC::isValidURL($opt['FS']))
				AND (!isset($this->embeddedfiles[basename($opt['FS'])]))) {
				$this->embeddedfiles[basename($opt['FS'])] = array('f' => ++$this->n, 'n' => ++$this->n, 'file' => $opt['FS']);
			}
		}
		// Add widgets annotation's icons
		if (isset($opt['mk']['i']) AND @ETLDEVTCPDF_STATIC::file_exists($opt['mk']['i'])) {
			$this->Image($opt['mk']['i'], '', '', 10, 10, '', '', '', false, 300, '', false, false, 0, false, true);
		}
		if (isset($opt['mk']['ri']) AND @ETLDEVTCPDF_STATIC::file_exists($opt['mk']['ri'])) {
			$this->Image($opt['mk']['ri'], '', '', 0, 0, '', '', '', false, 300, '', false, false, 0, false, true);
		}
		if (isset($opt['mk']['ix']) AND @ETLDEVTCPDF_STATIC::file_exists($opt['mk']['ix'])) {
	...
	protected function _putEmbeddedFiles() {
		...
		foreach ($this->embeddedfiles as $filename => $filedata) {
			$data = ETLDEVTCPDF_STATIC::fileGetContents($filedata['file']);
			if ($data !== FALSE) {
				$rawsize = strlen($data);
				if ($rawsize > 0) {
					// update name tree
	...
	protected function checkPageBreak($h=0, $y='', $addpage=true) {
		if (ETLDEVTCPDF_STATIC::empty_string($y)) {
			$y = $this->y;
		}
	...
	protected function getCellCode($w, $h=0, $txt='', $border=0, $ln=0, $align='', $fill=false, $link='', $stretch=0, $ignore_min_height=false, $calign='T', $valign='M') {
		...
		$txt = str_replace(ETLDEVTCPDF_FONTS::unichr(160, $this->isunicode), ' ', $txt);
		$prev_cell_margin = $this->cell_margin;
		$prev_cell_padding = $this->cell_padding;
		$txt = ETLDEVTCPDF_STATIC::removeSHY($txt, $this->isunicode);
		...
		if (ETLDEVTCPDF_STATIC::empty_string($w) OR ($w <= 0)) {
			if ($this->rtl) {
				$w = $x - $this->lMargin;
			} else {
		...
		if ($txt != '') {
			...
			if ($this->isunicode) {
				if (($this->CurrentFont['type'] == 'core') OR ($this->CurrentFont['type'] == 'TrueType') OR ($this->CurrentFont['type'] == 'Type1')) {
					$txt2 = ETLDEVTCPDF_FONTS::UTF8ToLatin1($txt2, $this->isunicode, $this->CurrentFont);
				} else {
					$unicode = ETLDEVTCPDF_FONTS::UTF8StringToArray($txt, $this->isunicode, $this->CurrentFont); // array of UTF-8 unicode values
					$unicode = ETLDEVTCPDF_FONTS::utf8Bidi($unicode, '', $this->tmprtl, $this->isunicode, $this->CurrentFont);
					...
					$txt2 = ETLDEVTCPDF_FONTS::arrUTF8ToUTF16BE($unicode, false);
				}
			}
			$txt2 = ETLDEVTCPDF_STATIC::_escape($txt2);
			...
			if (isset($uniblock)) {
				...
				foreach ($uniblock as $uk => $uniarr) {
					if (($uk % 2) == 0) {
						...
						// character to print
						$topchr = ETLDEVTCPDF_FONTS::arrUTF8ToUTF16BE($uniarr, false);
						$topchr = ETLDEVTCPDF_STATIC::_escape($topchr);
	...
	public function MultiCell($w, $h, $txt, $border=0, $align='J', $fill=false, $ln=1, $x='', $y='', $reseth=true, $stretch=0, $ishtml=false, $autopadding=true, $maxh=0, $valign='T', $fitcell=false,$rotated90Degrees=false) {
		...
		$this->setCellMargins(0, 0, 0, 0);
		if (ETLDEVTCPDF_STATIC::empty_string($this->lasth) OR $reseth) {
			// reset row height
			$this->resetLastH();
		}
		if (!ETLDEVTCPDF_STATIC::empty_string($y)) {
		...
		if (!ETLDEVTCPDF_STATIC::empty_string($x)) {
			$this->SetX($x);
		} else {
			$x = $this->GetX();
		}
		...
		// get border modes
		$border_start = ETLDEVTCPDF_STATIC::getBorderMode($border, $position='start', $this->opencell);
		$border_end = ETLDEVTCPDF_STATIC::getBorderMode($border, $position='end', $this->opencell);
		$border_middle = ETLDEVTCPDF_STATIC::getBorderMode($border, $position='middle', $this->opencell);
	...
	public function getNumLines($txt, $w=0, $reseth=false, $autopadding=true, $cellpadding='', $border=0) {
		...
		$lines = 1;
		$sum = 0;
		$chars = ETLDEVTCPDF_FONTS::utf8Bidi(ETLDEVTCPDF_FONTS::UTF8StringToArray($txt, $this->isunicode, $this->CurrentFont), $txt, $this->tmprtl, $this->isunicode, $this->CurrentFont);
		...
		for ($i = 0; $i < $length; ++$i) {
			...
			if (($c != 160)
					AND (($c == 173)
						OR preg_match($this->re_spaces, ETLDEVTCPDF_FONTS::unichr($c, $this->isunicode))
						OR (($c == 45)
							AND ($i > 0) AND ($i < ($length - 1))
							AND @preg_match('/[\p{L}]/'.$this->re_space['m'], ETLDEVTCPDF_FONTS::unichr($chars[($i - 1)], $this->isunicode))
							AND @preg_match('/[\p{L}]/'.$this->re_space['m'], ETLDEVTCPDF_FONTS::unichr($chars[($i + 1)], $this->isunicode))
						)
					)
				) {
	...
	public function Write($h, $txt, $link='', $fill=false, $align='', $ln=false, $stretch=0, $firstline=false, $firstblock=false, $maxh=0, $wadj=0, $margin='') {
		...
		if (preg_match(ETLDEVTCPDF_FONT_DATA::$uni_RE_PATTERN_ARABIC, $s)) {
			$arabic = true;
		} else {
			$arabic = false;
		}
		// check if string contains RTL text
		if ($arabic OR ($this->tmprtl == 'R') OR preg_match(ETLDEVTCPDF_FONT_DATA::$uni_RE_PATTERN_RTL, $s)) {
			$rtlmode = true;
		} else {
			$rtlmode = false;
		}
		...
		$chars = ETLDEVTCPDF_FONTS::UTF8StringToArray($s, $this->isunicode, $this->CurrentFont);
		// calculate maximum width for a single character on string
		$chrw = $this->GetArrStringWidth($chars, '', '', 0, true);
		array_walk($chrw, array($this, 'getRawCharWidth'));
		$maxchwidth = max($chrw);
		// get array of chars
		$uchars = ETLDEVTCPDF_FONTS::UTF8ArrayToUniArray($chars, $this->isunicode);
		// get the number of characters
		$nb = count($chars);
		// replacement for SHY character (minus symbol)
		$shy_replacement = 45;
		$shy_replacement_char = ETLDEVTCPDF_FONTS::unichr($shy_replacement, $this->isunicode);
		...
		while ($i < $nb) {
			...
			if ($c == 10) { // 10 = "\n" = new line
				...
				$tmpstr = ETLDEVTCPDF_FONTS::UniArrSubString($uchars, $j, $i);
				if ($firstline) {
					$startx = $this->x;
					$tmparr = array_slice($chars, $j, ($i - $j));
					if ($rtlmode) {
						$tmparr = ETLDEVTCPDF_FONTS::utf8Bidi($tmparr, $tmpstr, $this->tmprtl, $this->isunicode, $this->CurrentFont);
					}
				...
				if ($firstline) {
					...
					return (ETLDEVTCPDF_FONTS::UniArrSubString($uchars, $i));
				}
				++$nl;
				$j = $i + 1;
				$l = 0;
				...
				if (($c != 160)
					AND (($c == 173)
						...
						OR (($c == 45)
							AND ($i < ($nb - 1))
							AND @preg_match('/[\p{L}]/'.$this->re_space['m'], ETLDEVTCPDF_FONTS::unichr($pc, $this->isunicode))
							AND @preg_match('/[\p{L}]/'.$this->re_space['m'], ETLDEVTCPDF_FONTS::unichr($chars[($i + 1)], $this->isunicode))
						)
					)
				) {
				...
				if ($this->isUnicodeFont() AND ($arabic)) {
					...
					// *** very slow ***
					$l = $this->GetArrStringWidth(ETLDEVTCPDF_FONTS::utf8Bidi(array_slice($chars, $j, ($i - $j)), '', $this->tmprtl, $this->isunicode, $this->CurrentFont));
				} else {
				...
				if (($l > $wmax) OR (($c == 173) AND (($l + $tmp_shy_replacement_width) >= $wmax))) {
					...
					if ($sep == -1) {
						...
						if (($this->rtl AND ($this->x <= ($this->w - $this->rMargin - $this->cell_padding['R'] - $margin['R'] - $chrwidth)))
							...
							$linebreak = true;
							if ($firstline) {
								return (ETLDEVTCPDF_FONTS::UniArrSubString($uchars, $j));
							}
						} else {
						...
						$strrest = ETLDEVTCPDF_FONTS::UniArrSubString($uchars, ($sep + $endspace));
						$nextstr = ETLDEVTCPDF_STATIC::pregSplit('/'.$this->re_space['p'].'/', $this->re_space['m'], $this->stringTrim($strrest));
						if (isset($nextstr[0]) AND ($this->GetStringWidth($nextstr[0]) > $pw)) {
							...
							$tmpstr = ETLDEVTCPDF_FONTS::UniArrSubString($uchars, $j, ($sep + $endspace));
							if ($firstline) {
								$startx = $this->x;
							...
							if ($firstline) {
								...
								$this->cell_padding = $tmpcellpadding;
								return (ETLDEVTCPDF_FONTS::UniArrSubString($uchars, ($sep + $endspace)));
							}
							$i = $sep;
							$sep = -1;
							$shy = false;
		...
		if ($l > 0) {
			...
			$tmpstr = ETLDEVTCPDF_FONTS::UniArrSubString($uchars, $j, $nb);
			if ($firstline) {
				$startx = $this->x;
			...
			if ($firstline) {
				$this->cell_padding = $tmpcellpadding;
				return (ETLDEVTCPDF_FONTS::UniArrSubString($uchars, $nb));
			}
			++$nl;
		}
		if ($firstline) {
	...
	public function Image($file, $x='', $y='', $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='', $ismask=false, $imgmask=false, $border=0, $fitbox=false, $hidden=false, $fitonpage=false, $alt=false, $altimgs=array()) {
		...
		if ($file[0] === '@') {
			...
			if (!@ETLDEVTCPDF_STATIC::file_exists($file)) {
				return false;
			}
			if (($imsize = @getimagesize($file)) === FALSE) {
				...
				} elseif (strpos($file, '__etldevtcpdf_'.$this->file_id.'_img') === FALSE) {
					$imgdata = ETLDEVTCPDF_STATIC::fileGetContents($file);
				}
			}
		}
		if (!empty($imgdata)) {
			// copy image to cache
			$original_file = $file;
			$file = ETLDEVTCPDF_STATIC::getObjFilename('img', $this->file_id);
			$fp = ETLDEVTCPDF_STATIC::fopenLocal($file, 'w');
			if (!$fp) {
		...
		if (in_array($file, $this->imagekeys)) {
			...
			// get existing image data
			$info = $this->getImageBuffer($file);
			if (strpos($file, '__etldevtcpdf_'.$this->file_id.'_imgmask_') === FALSE) {
		...
		} elseif (($ismask === false) AND ($imgmask === false) AND (strpos($file, '__etldevtcpdf_'.$this->file_id.'_imgmask_') === FALSE)) {
			// create temp image file (without alpha channel)
			$tempfile_plain = K_PATH_CACHE.'__etldevtcpdf_'.$this->file_id.'_imgmask_plain_'.$filehash;
			// create temp alpha file
			$tempfile_alpha = K_PATH_CACHE.'__etldevtcpdf_'.$this->file_id.'_imgmask_alpha_'.$filehash;
		...
		if ($newimage) {
			...
			if ($type == '') {
				$type = ETLDEVTCPDF_IMAGES::getImageFileType($file, $imsize);
			} elseif ($type == 'jpg') {
				$type = 'jpeg';
			}
			$mqr = ETLDEVTCPDF_STATIC::get_mqr();
			ETLDEVTCPDF_STATIC::set_mqr(false);
			// Specific image handlers (defined on ETLDEVTCPDF_IMAGES CLASS)
			$mtd = '_parse'.$type;
			// GD image handler function
			$gdfunction = 'imagecreatefrom'.$type;
			$info = false;
			if ((method_exists('ETLDEVTCPDF_IMAGES', $mtd)) AND (!($resize AND (function_exists($gdfunction) OR extension_loaded('imagick'))))) {
				// ETLDEVTCPDF image functions
				$info = ETLDEVTCPDF_IMAGES::$mtd($file);
				if (($ismask === false) AND ($imgmask === false) AND (strpos($file, '__etldevtcpdf_'.$this->file_id.'_imgmask_') === FALSE)
					...
					if ($img !== false) {
						if ($resize) {
							...
							if (($type == 'gif') OR ($type == 'png')) {
								$imgr = ETLDEVTCPDF_IMAGES::setGDImageTransparency($imgr, $img);
							}
							imagecopyresampled($imgr, $img, 0, 0, 0, 0, $neww, $newh, $pixw, $pixh);
							$img = $imgr;
						}
						if (($type == 'gif') OR ($type == 'png')) {
							$info = ETLDEVTCPDF_IMAGES::_toPNG($img, ETLDEVTCPDF_STATIC::getObjFilename('img', $this->file_id));
						} else {
							$info = ETLDEVTCPDF_IMAGES::_toJPEG($img, $this->jpeg_quality, ETLDEVTCPDF_STATIC::getObjFilename('img', $this->file_id));
						}
					}
				} catch(Exception $e) {
					$info = false;
				...
				try {
					...
					if ($type == 'svg') {
						if ($file[0] === '@') {
							...
							$svgimg = ETLDEVTCPDF_STATIC::fileGetContents($file);
						}
						if ($svgimg !== FALSE) {
					...
					$tempname = ETLDEVTCPDF_STATIC::getObjFilename('img', $this->file_id);
					$img->writeImage($tempname);
					$info = ETLDEVTCPDF_IMAGES::_parsejpeg($tempname);
					unlink($tempname);
					$img->destroy();
				} catch(Exception $e) {
					$info = false;
			...
			ETLDEVTCPDF_STATIC::set_mqr($mqr);
			if ($ismask) {
				// force grayscale
				$info['cs'] = 'DeviceGray';
			}
	...
	protected function ImagePngAlpha($file, $x, $y, $wpx, $hpx, $w, $h, $type, $link, $align, $resize, $dpi, $palign, $filehash='') {
		...
		if (($parsed === false) AND extension_loaded('imagick')) {
			try {
				...
				$img->readImage($file);
				// clone image object
				$imga = ETLDEVTCPDF_STATIC::objclone($img);
		...
		if ($parsed === false) {
			if (empty($parse_error)) {
				$this->Error('ETLDEVTCPDF requires the Imagick or GD extension to handle PNG images with alpha channel.');
			} else {
	...
	public function Output($name='doc.pdf', $dest='I') {
		...
		if ($this->sign) {
			...
			// remove filler space
			$byterange_string_len = strlen(ETLDEVTCPDF_STATIC::$byterange_string);
			// define the ByteRange
			$byte_range = array();
			$byte_range[0] = 0;
			$byte_range[1] = strpos($pdfdoc, ETLDEVTCPDF_STATIC::$byterange_string) + $byterange_string_len + 10;
			...
			$pdfdoc = str_replace(ETLDEVTCPDF_STATIC::$byterange_string, $byterange, $pdfdoc);
			// write the document to a temporary folder
			$tempdoc = ETLDEVTCPDF_STATIC::getObjFilename('doc', $this->file_id);
			$f = ETLDEVTCPDF_STATIC::fopenLocal($tempdoc, 'wb');
			if (!$f) {
			...
			$tempsign = ETLDEVTCPDF_STATIC::getObjFilename('sig', $this->file_id);
		...
		switch($dest) {
			case 'I': {
				...
				if (php_sapi_name() != 'cli') {
					...
					ETLDEVTCPDF_STATIC::sendOutputData($this->getBuffer(), $this->bufferlen);
				} else {
					echo $this->getBuffer();
				}
				break;
			...
			case 'FD': {
				...
				$f = ETLDEVTCPDF_STATIC::fopenLocal($name, 'wb');
				if (!$f) {
				...
				if ($dest == 'FI') {
					...
					header('Last-Modified: '.gmdate('D, d M Y H:i:s').' GMT');
					header('Content-Disposition: inline; filename="'.basename($name).'"');
					ETLDEVTCPDF_STATIC::sendOutputData(file_get_contents($name), filesize($name));
				} elseif ($dest == 'FD') {
					// send headers to browser
					if (ob_get_contents()) {
						$this->Error('Some data has already been output, can\'t send PDF file');
		...
		if ($destroyall AND !$preserve_objcopy) {
			...
			if ($handle = @opendir(K_PATH_CACHE)) {
				while ( false !== ( $file_name = readdir( $handle ) ) ) {
					if (strpos($file_name, '__etldevtcpdf_'.$this->file_id.'_') === 0) {
	...
	protected function getInternalPageNumberAliases($a= '') {
		...
		$u = '{'.$a.'}';
		$alias['u'][] = ETLDEVTCPDF_STATIC::_escape($u);
		if ($this->isunicode) {
			$alias['u'][] = ETLDEVTCPDF_STATIC::_escape(ETLDEVTCPDF_FONTS::UTF8ToLatin1($u, $this->isunicode, $this->CurrentFont));
			$alias['u'][] = ETLDEVTCPDF_STATIC::_escape(ETLDEVTCPDF_FONTS::utf8StrRev($u, false, $this->tmprtl, $this->isunicode, $this->CurrentFont));
			$alias['a'][] = ETLDEVTCPDF_STATIC::_escape(ETLDEVTCPDF_FONTS::UTF8ToLatin1($a, $this->isunicode, $this->CurrentFont));
			$alias['a'][] = ETLDEVTCPDF_STATIC::_escape(ETLDEVTCPDF_FONTS::utf8StrRev($a, false, $this->tmprtl, $this->isunicode, $this->CurrentFont));
		}
		$alias['a'][] = ETLDEVTCPDF_STATIC::_escape($a);
		return $alias;
	}
	...
	protected function getAllInternalPageNumberAliases() {
		$basic_alias = array(ETLDEVTCPDF_STATIC::$alias_tot_pages, ETLDEVTCPDF_STATIC::$alias_num_page, ETLDEVTCPDF_STATIC::$alias_group_tot_pages, ETLDEVTCPDF_STATIC::$alias_group_num_page, ETLDEVTCPDF_STATIC::$alias_right_shift);
		$pnalias = array();
			...
			foreach ($alias as $a) {
				...
				if (($pos = strpos($page, $a)) !== false) {
					...
					if ($type == 'u') {
						...
						$shift = ETLDEVTCPDF_FONTS::UTF8ToUTF16BE($shift, false, $this->isunicode, $this->CurrentFont);
					} else {
	...
	protected function setPageBoxTypes($boxes) {
		...
		foreach ($boxes as $box) {
			if (in_array($box, ETLDEVTCPDF_STATIC::$pageboxes)) {
				$this->page_boxes[] = $box;
			}
		}
	}
	...
	protected function _putpages() {
		...
		$ptpa = ETLDEVTCPDF_STATIC::formatPageNumber(($this->starting_page_number + $num_pages - 1));
		$ptpu = ETLDEVTCPDF_FONTS::UTF8ToUTF16BE($ptpa, false, $this->isunicode, $this->CurrentFont);
		...
		for ($n = 1; $n <= $num_pages; ++$n) {
			...
			$pnpa = ETLDEVTCPDF_STATIC::formatPageNumber(($this->starting_page_number + $n - 1));
			$pnpu = ETLDEVTCPDF_FONTS::UTF8ToUTF16BE($pnpa, false, $this->isunicode, $this->CurrentFont);
			...
			if (!empty($this->pagegroups)) {
				if (isset($this->newpagegroup[$n])) {
					$pagegroupnum = 0;
					++$groupnum;
					$ptga = ETLDEVTCPDF_STATIC::formatPageNumber($this->pagegroups[$groupnum]);
					$ptgu = ETLDEVTCPDF_FONTS::UTF8ToUTF16BE($ptga, false, $this->isunicode, $this->CurrentFont);
					$ptg_num_chars = $this->GetNumChars($ptga);
				}
				++$pagegroupnum;
				$pnga = ETLDEVTCPDF_STATIC::formatPageNumber($pagegroupnum);
				$pngu = ETLDEVTCPDF_FONTS::UTF8ToUTF16BE($pnga, false, $this->isunicode, $this->CurrentFont);
				...
				list($temppage, $gdiff) = ETLDEVTCPDF_STATIC::replacePageNumAliases($temppage, $replace, $gdiff);
			}
			// replace page numbers
			$replace = array();
			...
			list($temppage, $pdiff) = ETLDEVTCPDF_STATIC::replacePageNumAliases($temppage, $replace, $pdiff);
			...
			if (isset($this->PageAnnots[$n])) {
				...
				foreach ($this->PageAnnots[$n] as $key => $pl) {
					...
					if (isset($pl['opt']['c']) AND (is_array($pl['opt']['c'])) AND !empty($pl['opt']['c'])) {
						$annots .= ' /C '.ETLDEVTCPDF_COLORS::getColorStringFromArray($pl['opt']['c']);
					}
					//$annots .= ' /StructParent ';
					//$annots .= ' /OC ';
						...
						case 'link': {
							if (is_string($pl['txt']) && !empty($pl['txt'])) {
								if ($pl['txt'][0] == '#') {
									...
									$annots .= ' /A <</S /GoTo /D '.ETLDEVTCPDF_STATIC::encodeNameObject(substr($pl['txt'], 1)).'>>';
								} elseif ($pl['txt'][0] == '%') {
									// embedded PDF file
						...
						case 'widget': {
							...
							if (isset($pl['opt']['mk']) AND (is_array($pl['opt']['mk'])) AND !empty($pl['opt']['mk'])) {
								...
								if (isset($pl['opt']['mk']['bc']) AND (is_array($pl['opt']['mk']['bc']))) {
									$annots .= ' /BC '.ETLDEVTCPDF_COLORS::getColorStringFromArray($pl['opt']['mk']['bc']);
								}
								if (isset($pl['opt']['mk']['bg']) AND (is_array($pl['opt']['mk']['bg']))) {
									$annots .= ' /BG '.ETLDEVTCPDF_COLORS::getColorStringFromArray($pl['opt']['mk']['bg']);
								}
	...
	protected function _putfonts() {
		...
		foreach ($this->FontFiles as $file => $info) {
			...
			$fontfile = ETLDEVTCPDF_FONTS::getFontFullPath($file, $info['fontdir']);
			if (!ETLDEVTCPDF_STATIC::empty_string($fontfile)) {
				...
				if ((!$compressed) AND (isset($info['length2']))) {
					...
					$font = ETLDEVTCPDF_FONTS::_getTrueTypeFontSubset($font, $subsetchars);
		...
		foreach ($this->fontkeys as $k) {
			...
			if ($type == 'core') {
				...
				if (!ETLDEVTCPDF_STATIC::empty_string($font['file'])) {
	...
	protected function _puttruetypeunicode($font) {
		...
		$stream = ETLDEVTCPDF_FONT_DATA::$uni_identity_h;
		// ToUnicode Object
		$this->_newobj();
		...
		$out .= "\n".ETLDEVTCPDF_FONTS::_putfontwidths($font, 0);
		if (isset($font['ctg']) AND (!ETLDEVTCPDF_STATIC::empty_string($font['ctg']))) {
		...
		if (isset($font['ctg']) AND (!ETLDEVTCPDF_STATIC::empty_string($font['ctg']))) {
			...
			// search and get ctg font file to embedd
			$fontfile = ETLDEVTCPDF_FONTS::getFontFullPath($ctgfile, $fontdir);
			if (ETLDEVTCPDF_STATIC::empty_string($fontfile)) {
	...
	protected function _putcidfont0($font) {
		...
		$out .= ' /FontDescriptor '.($this->n + 1).' 0 R';
		$out .= ' /DW '.$font['dw'];
		$out .= "\n".ETLDEVTCPDF_FONTS::_putfontwidths($font, $cidoffset);
		$out .= ' >>';
		$out .= "\n".'endobj';
		$this->_out($out);
		$this->_newobj();
	...
	protected function _putinfo() {
		...
		if (!ETLDEVTCPDF_STATIC::empty_string($this->title)) {
			// The document's title.
			$out .= ' /Title '.$this->_textstring($this->title, $oid);
		}
		if (!ETLDEVTCPDF_STATIC::empty_string($this->author)) {
			// The name of the person who created the document.
			$out .= ' /Author '.$this->_textstring($this->author, $oid);
		}
		if (!ETLDEVTCPDF_STATIC::empty_string($this->subject)) {
			// The subject of the document.
			$out .= ' /Subject '.$this->_textstring($this->subject, $oid);
		}
		if (!ETLDEVTCPDF_STATIC::empty_string($this->keywords)) {
			// Keywords associated with the document.
			$out .= ' /Keywords '.$this->_textstring($this->keywords, $oid);
		}
		if (!ETLDEVTCPDF_STATIC::empty_string($this->creator)) {
		...
		// default producer
		$out .= ' /Producer '.$this->_textstring(ETLDEVTCPDF_STATIC::getTCPDFProducer(), $oid);
	...
	protected function _putXMP() {
		...
		$this->encrypted = false;
		// set XMP data
		$xmp = '<?xpacket begin="'.ETLDEVTCPDF_FONTS::unichr(0xfeff, $this->isunicode).'" id="W5M0MpCehiHzreSzNTczkc9d"?>'."\n";
		...
		$xmp .= "\t\t\t".'<dc:title>'."\n";
		$xmp .= "\t\t\t\t".'<rdf:Alt>'."\n";
		$xmp .= "\t\t\t\t\t".'<rdf:li xml:lang="x-default">'.ETLDEVTCPDF_STATIC::_escapeXML($this->title).'</rdf:li>'."\n";
		$xmp .= "\t\t\t\t".'</rdf:Alt>'."\n";
		$xmp .= "\t\t\t".'</dc:title>'."\n";
		$xmp .= "\t\t\t".'<dc:creator>'."\n";
		$xmp .= "\t\t\t\t".'<rdf:Seq>'."\n";
		$xmp .= "\t\t\t\t\t".'<rdf:li>'.ETLDEVTCPDF_STATIC::_escapeXML($this->author).'</rdf:li>'."\n";
		$xmp .= "\t\t\t\t".'</rdf:Seq>'."\n";
		$xmp .= "\t\t\t".'</dc:creator>'."\n";
		$xmp .= "\t\t\t".'<dc:description>'."\n";
		$xmp .= "\t\t\t\t".'<rdf:Alt>'."\n";
		$xmp .= "\t\t\t\t\t".'<rdf:li xml:lang="x-default">'.ETLDEVTCPDF_STATIC::_escapeXML($this->subject).'</rdf:li>'."\n";
		$xmp .= "\t\t\t\t".'</rdf:Alt>'."\n";
		$xmp .= "\t\t\t".'</dc:description>'."\n";
		$xmp .= "\t\t\t".'<dc:subject>'."\n";
		$xmp .= "\t\t\t\t".'<rdf:Bag>'."\n";
		$xmp .= "\t\t\t\t\t".'<rdf:li>'.ETLDEVTCPDF_STATIC::_escapeXML($this->keywords).'</rdf:li>'."\n";
		$xmp .= "\t\t\t\t".'</rdf:Bag>'."\n";
		$xmp .= "\t\t\t".'</dc:subject>'."\n";
		$xmp .= "\t\t".'</rdf:Description>'."\n";
		// convert doc creation date format
		$dcdate = ETLDEVTCPDF_STATIC::getFormattedDate($this->doc_creation_timestamp);
		...
		$doccreationdate = ETLDEVTCPDF_STATIC::_escapeXML($doccreationdate);
		// convert doc modification date format
		$dmdate = ETLDEVTCPDF_STATIC::getFormattedDate($this->doc_modification_timestamp);
		...
		$docmoddate = ETLDEVTCPDF_STATIC::_escapeXML($docmoddate);
		...
		$xmp .= "\t\t\t".'<pdf:Keywords>'.ETLDEVTCPDF_STATIC::_escapeXML($this->keywords).'</pdf:Keywords>'."\n";
		$xmp .= "\t\t\t".'<pdf:Producer>'.ETLDEVTCPDF_STATIC::_escapeXML(ETLDEVTCPDF_STATIC::getTCPDFProducer()).'</pdf:Producer>'."\n";
		$xmp .= "\t\t".'</rdf:Description>'."\n";
	...
	protected function _putcatalog() {
		...
		if (isset($this->LayoutMode) AND (!ETLDEVTCPDF_STATIC::empty_string($this->LayoutMode))) {
			$out .= ' /PageLayout /'.$this->LayoutMode;
		}
		if (isset($this->PageMode) AND (!ETLDEVTCPDF_STATIC::empty_string($this->PageMode))) {
		...
		if (!empty($this->pdflayers)) {
			...
			$out .= ' /Creator '.$this->_textstring('ETLDEVTCPDF', $oid);
			$out .= ' /BaseState /ON';
			$out .= ' /OFF ['.$lyrobjs_off.']';
	...
	protected function _putviewerpreferences() {
		...
		if (isset($vp['Duplex']) AND (!ETLDEVTCPDF_STATIC::empty_string($vp['Duplex']))) {
	...
	protected function _beginpage($orientation='', $format='') {
		...
		$this->state = 2;
		if (ETLDEVTCPDF_STATIC::empty_string($orientation)) {
			if (isset($this->CurOrientation)) {
		...
		if (ETLDEVTCPDF_STATIC::empty_string($format)) {
	...
	protected function _datastring($s, $n=0) {
		...
		return '('. ETLDEVTCPDF_STATIC::_escape($s).')';
	}
	...
	public function setDocCreationTimestamp($time) {
		if (is_string($time)) {
			$time = ETLDEVTCPDF_STATIC::getTimestamp($time);
		}
	...
	protected function _datestring($n=0, $timestamp=0) {
		...
		return $this->_datastring('D:'.ETLDEVTCPDF_STATIC::getFormattedDate($timestamp), $n);
	}
	...
	protected function _textstring($s, $n=0) {
		if ($this->isunicode) {
			...
			$s = ETLDEVTCPDF_FONTS::UTF8ToUTF16BE($s, true, $this->isunicode, $this->CurrentFont);
		}
	...
	protected function _objectkey($n) {
		...
		$objkey = substr(ETLDEVTCPDF_STATIC::_md5_16($objkey), 0, (($this->encryptdata['Length'] / 8) + 5));
	...
	protected function _encrypt_data($n, $s) {
		...
		switch ($this->encryptdata['mode']) {
			case 0:   // RC4-40
			case 1: { // RC4-128
				$s = ETLDEVTCPDF_STATIC::_RC4($this->_objectkey($n), $s, $this->last_enc_key, $this->last_enc_key_c);
				break;
			}
			case 2: { // AES-128
				$s = ETLDEVTCPDF_STATIC::_AES($this->_objectkey($n), $s);
				break;
			}
			case 3: { // AES-256
				$s = ETLDEVTCPDF_STATIC::_AES($this->encryptdata['key'], $s);
				break;
			}
		}
		return $s;
	...
	protected function _putencryption() {
		...
		if ($this->encryptdata['pubkey']) {
			...
			if ($this->encryptdata['V'] == 5) { // AES-256
				$out .= ' 5';
				$out .= ' /OE ('.ETLDEVTCPDF_STATIC::_escape($this->encryptdata['OE']).')';
				$out .= ' /UE ('.ETLDEVTCPDF_STATIC::_escape($this->encryptdata['UE']).')';
				$out .= ' /Perms ('.ETLDEVTCPDF_STATIC::_escape($this->encryptdata['perms']).')';
			...
			$out .= ' /O ('.ETLDEVTCPDF_STATIC::_escape($this->encryptdata['O']).')';
			$out .= ' /U ('.ETLDEVTCPDF_STATIC::_escape($this->encryptdata['U']).')';
	...
	protected function _Uvalue() {
		if ($this->encryptdata['mode'] == 0) { // RC4-40
			return ETLDEVTCPDF_STATIC::_RC4($this->encryptdata['key'], ETLDEVTCPDF_STATIC::$enc_padding, $this->last_enc_key, $this->last_enc_key_c);
		} elseif ($this->encryptdata['mode'] < 3) { // RC4-128, AES-128
			$tmp = ETLDEVTCPDF_STATIC::_md5_16(ETLDEVTCPDF_STATIC::$enc_padding.$this->encryptdata['fileid']);
			$enc = ETLDEVTCPDF_STATIC::_RC4($this->encryptdata['key'], $tmp, $this->last_enc_key, $this->last_enc_key_c);
			$len = strlen($tmp);
			for ($i = 1; $i <= 19; ++$i) {
				$ek = '';
				for ($j = 0; $j < $len; ++$j) {
				...
				$enc = ETLDEVTCPDF_STATIC::_RC4($ek, $enc, $this->last_enc_key, $this->last_enc_key_c);
			}
			$enc .= str_repeat("\x00", 16);
			return substr($enc, 0, 32);
		} elseif ($this->encryptdata['mode'] == 3) { // AES-256
			$seed = ETLDEVTCPDF_STATIC::_md5_16(ETLDEVTCPDF_STATIC::getRandomSeed());
	...
	protected function _UEvalue() {
		...
		return ETLDEVTCPDF_STATIC::_AESnopad($hashkey, $this->encryptdata['key']);
	}
	...
	protected function _Ovalue() {
		if ($this->encryptdata['mode'] < 3) { // RC4-40, RC4-128, AES-128
			$tmp = ETLDEVTCPDF_STATIC::_md5_16($this->encryptdata['owner_password']);
			if ($this->encryptdata['mode'] > 0) {
				for ($i = 0; $i < 50; ++$i) {
					$tmp = ETLDEVTCPDF_STATIC::_md5_16($tmp);
				}
			}
			$owner_key = substr($tmp, 0, ($this->encryptdata['Length'] / 8));
			$enc = ETLDEVTCPDF_STATIC::_RC4($owner_key, $this->encryptdata['user_password'], $this->last_enc_key, $this->last_enc_key_c);
			if ($this->encryptdata['mode'] > 0) {
	...
	protected function _fixAES256Password($password) {
		...
		$psw_array = ETLDEVTCPDF_FONTS::utf8Bidi(ETLDEVTCPDF_FONTS::UTF8StringToArray($password, $this->isunicode, $this->CurrentFont), $password, $this->rtl, $this->isunicode, $this->CurrentFont);
		foreach ($psw_array as $c) {
			$psw .= ETLDEVTCPDF_FONTS::unichr($c, $this->isunicode);
		}
	...
	protected function _generateencryptionkey() {
		...
		if (!$this->encryptdata['pubkey']) { // standard mode
			if ($this->encryptdata['mode'] == 3) { // AES-256
				...
				$this->encryptdata['key'] = substr(hash('sha256', ETLDEVTCPDF_STATIC::getRandomSeed(), true), 0, $keybytelen);
				// truncate passwords
				...
				$perms = ETLDEVTCPDF_STATIC::getEncPermissionsString($this->encryptdata['protection']); // bytes 0-3
				...
				$this->encryptdata['perms'] = ETLDEVTCPDF_STATIC::_AESnopad($this->encryptdata['key'], $perms);
			} else { // RC4-40, RC4-128, AES-128
				// Pad passwords
				$this->encryptdata['user_password'] = substr($this->encryptdata['user_password'].ETLDEVTCPDF_STATIC::$enc_padding, 0, 32);
				$this->encryptdata['owner_password'] = substr($this->encryptdata['owner_password'].ETLDEVTCPDF_STATIC::$enc_padding, 0, 32);
				// Compute O value
				$this->encryptdata['O'] = $this->_Ovalue();
				// get default permissions (reverse byte order)
				$permissions = ETLDEVTCPDF_STATIC::getEncPermissionsString($this->encryptdata['protection']);
				// Compute encryption key
				$tmp = ETLDEVTCPDF_STATIC::_md5_16($this->encryptdata['user_password'].$this->encryptdata['O'].$permissions.$this->encryptdata['fileid']);
				if ($this->encryptdata['mode'] > 0) {
					for ($i = 0; $i < 50; ++$i) {
						$tmp = ETLDEVTCPDF_STATIC::_md5_16(substr($tmp, 0, $keybytelen));
					}
				}
			...
			// random 20-byte seed
			$seed = sha1(ETLDEVTCPDF_STATIC::getRandomSeed(), true);
			$recipient_bytes = '';
			foreach ($this->encryptdata['pubkeys'] as $pubkey) {
				// for each public certificate
				if (isset($pubkey['p'])) {
					$pkprotection = ETLDEVTCPDF_STATIC::getUserPermissionCode($pubkey['p'], $this->encryptdata['mode']);
				} else {
					$pkprotection = $this->encryptdata['protection'];
				}
				// get default permissions (reverse byte order)
				$pkpermissions = ETLDEVTCPDF_STATIC::getEncPermissionsString($pkprotection);
				// envelope data
				$envelope = $seed.$pkpermissions;
				// write the envelope data to a temporary file
				$tempkeyfile = ETLDEVTCPDF_STATIC::getObjFilename('key', $this->file_id);
				$f = ETLDEVTCPDF_STATIC::fopenLocal($tempkeyfile, 'wb');
				if (!$f) {
				...
				fclose($f);
				$tempencfile = ETLDEVTCPDF_STATIC::getObjFilename('enc', $this->file_id);
	 ...
	 * @param $pubkeys (String) array of recipients containing public-key certificates ('c') and permissions ('p'). For example: array(array('c' => 'file://../examples/data/cert/etldevtcpdf.crt', 'p' => array('print')))
	 * @public
	 * @since 2.0.000 (2008-01-02)
	 * @author Nicola Asuni
	 */
	public function SetProtection($permissions=array('print', 'modify', 'copy', 'annot-forms', 'fill-forms', 'extract', 'assemble', 'print-high'), $user_pass='', $owner_pass=null, $mode=0, $pubkeys=null) {
		...
		$this->encryptdata['protection'] = ETLDEVTCPDF_STATIC::getUserPermissionCode($permissions, $mode);
		...
		if ($owner_pass === null) {
			$owner_pass = md5(ETLDEVTCPDF_STATIC::getRandomSeed());
		}
		...
		$this->encrypted = true;
		$this->encryptdata['fileid'] = ETLDEVTCPDF_STATIC::convertHexStringToString($this->file_id);
	...
	public function Rect($x, $y, $w, $h, $style='', $border_style=array(), $fill_color=array()) {
		...
		if (!empty($style)) {
			$op = ETLDEVTCPDF_STATIC::getPathPaintOperator($style);
	...
	public function Ellipse($x0, $y0, $rx, $ry='', $angle=0, $astart=0, $afinish=360, $style='', $line_style=array(), $fill_color=array(), $nc=2) {
		...
		if (ETLDEVTCPDF_STATIC::empty_string($ry) OR ($ry == 0)) {
			$ry = $rx;
		}
		if (!(false === strpos($style, 'F')) AND isset($fill_color)) {
			$this->SetFillColorArray($fill_color);
	...
	public function setDestination($name, $y=-1, $page='', $x=-1) {
		...
		$name = ETLDEVTCPDF_STATIC::encodeNameObject($name);
		if (ETLDEVTCPDF_STATIC::empty_string($name)) {
			return false;
		}
		if ($y == -1) {
			$y = $this->GetY();
	...
	protected function _putbookmarks() {
		...
		//Outline items
		$n = $this->n + 1;
		$nltags = '/<br[\s]?\/>|<\/(blockquote|dd|dl|div|dt|h1|h2|h3|h4|h5|h6|hr|li|ol|p|pre|ul|etldevtcpdf|table|tr|td)>/si';
		foreach ($this->outlines as $i => $o) {
			$oid = $this->_newobj();
			...
			if (isset($o['u']) AND !empty($o['u'])) {
				...
				if (is_string($o['u'])) {
					if ($o['u'][0] == '#') {
						// internal destination
						$out .= ' /Dest /'.ETLDEVTCPDF_STATIC::encodeNameObject(substr($o['u'], 1));
					} elseif ($o['u'][0] == '%') {
						// embedded PDF file
	...
	protected function _addfield($type, $name, $x, $y, $w, $h, $prop) {
		...
		foreach($prop as $key => $val) {
			if (strcmp(substr($key, -5), 'Color') == 0) {
				$val = ETLDEVTCPDF_COLORS::_JScolor($val);
			} else {
				$val = "'".$val."'";
			}
	...
	public function TextField($name, $w, $h, $prop=array(), $opt=array(), $x='', $y='', $js=false) {
		...
		// get annotation data
		$popt = ETLDEVTCPDF_STATIC::getAnnotOptFromJSProp($prop, $this->spot_colors, $this->rtl);
	...
	public function RadioButton($name, $w, $prop=array(), $opt=array(), $onvalue='On', $checked=false, $x='', $y='', $js=false) {
		...
		if (ETLDEVTCPDF_STATIC::empty_string($onvalue)) {
			$onvalue = 'On';
		}
		if ($checked) {
			$defval = $onvalue;
	...
	protected function _putsignature() {
		...
		$out .= ' '.ETLDEVTCPDF_STATIC::$byterange_string;
		...
		if (empty($this->signature_data['approval']) OR ($this->signature_data['approval'] != 'A')) {
			...
			if ($this->signature_data['cert_type'] > 0) {
				...
				$out .= ' /Type /TransformParams';
				$out .= ' /V /2.2';
				if (!ETLDEVTCPDF_STATIC::empty_string($this->ur['document'])) {
					$out .= ' /Document['.$this->ur['document'].']';
				}
				if (!ETLDEVTCPDF_STATIC::empty_string($this->ur['form'])) {
					$out .= ' /Form['.$this->ur['form'].']';
				}
				if (!ETLDEVTCPDF_STATIC::empty_string($this->ur['signature'])) {
					$out .= ' /Signature['.$this->ur['signature'].']';
				}
				if (!ETLDEVTCPDF_STATIC::empty_string($this->ur['annots'])) {
					$out .= ' /Annots['.$this->ur['annots'].']';
				}
				if (!ETLDEVTCPDF_STATIC::empty_string($this->ur['ef'])) {
					$out .= ' /EF['.$this->ur['ef'].']';
				}
				if (!ETLDEVTCPDF_STATIC::empty_string($this->ur['formex'])) {
		...
		if (isset($this->signature_data['info']['Name']) AND !ETLDEVTCPDF_STATIC::empty_string($this->signature_data['info']['Name'])) {
			$out .= ' /Name '.$this->_textstring($this->signature_data['info']['Name'], $sigobjid);
		}
		if (isset($this->signature_data['info']['Location']) AND !ETLDEVTCPDF_STATIC::empty_string($this->signature_data['info']['Location'])) {
			$out .= ' /Location '.$this->_textstring($this->signature_data['info']['Location'], $sigobjid);
		}
		if (isset($this->signature_data['info']['Reason']) AND !ETLDEVTCPDF_STATIC::empty_string($this->signature_data['info']['Reason'])) {
			$out .= ' /Reason '.$this->_textstring($this->signature_data['info']['Reason'], $sigobjid);
		}
		if (isset($this->signature_data['info']['ContactInfo']) AND !ETLDEVTCPDF_STATIC::empty_string($this->signature_data['info']['ContactInfo'])) {
	 ...
	 * To create self-signed signature: openssl req -x509 -nodes -days 365000 -newkey rsa:1024 -keyout etldevtcpdf.crt -out etldevtcpdf.crt
	 * To export crt to p12: openssl pkcs12 -export -in etldevtcpdf.crt -out etldevtcpdf.p12
	 * To convert pfx certificate to pem: openssl pkcs12 -in etldevtcpdf.pfx -out etldevtcpdf.crt -nodes
	...
	public function getAliasRightShift() {
		...
		$ref = '{'.ETLDEVTCPDF_STATIC::$alias_right_shift.'}{'.ETLDEVTCPDF_STATIC::$alias_tot_pages.'}{'.ETLDEVTCPDF_STATIC::$alias_num_page.'}';
		...
		$alias = ETLDEVTCPDF_STATIC::$alias_right_shift.$sdiff.'}';
		if ($this->isUnicodeFont()) {
			$alias = '{'.$alias;
		}
		return $alias;
	...
	public function getAliasNbPages() {
		if ($this->isUnicodeFont()) {
			return '{'.ETLDEVTCPDF_STATIC::$alias_tot_pages.'}';
		}
		return ETLDEVTCPDF_STATIC::$alias_tot_pages;
	}
	...
	public function getAliasNumPage() {
		if ($this->isUnicodeFont()) {
			return '{'.ETLDEVTCPDF_STATIC::$alias_num_page.'}';
		}
		return ETLDEVTCPDF_STATIC::$alias_num_page;
	}
	...
	public function getPageGroupAlias() {
		if ($this->isUnicodeFont()) {
			return '{'.ETLDEVTCPDF_STATIC::$alias_group_tot_pages.'}';
		}
		return ETLDEVTCPDF_STATIC::$alias_group_tot_pages;
	}
	...
	public function getPageNumGroupAlias() {
		if ($this->isUnicodeFont()) {
			return '{'.ETLDEVTCPDF_STATIC::$alias_group_num_page.'}';
		}
		return ETLDEVTCPDF_STATIC::$alias_group_num_page;
	}
	...
	public function getGroupPageNoFormatted() {
		return ETLDEVTCPDF_STATIC::formatPageNumber($this->getGroupPageNo());
	}
	...
	public function PageNoFormatted() {
		return ETLDEVTCPDF_STATIC::formatPageNumber($this->PageNo());
	}
	...
	public function setOverprint($stroking=true, $nonstroking='', $mode=0) {
		...
		if (ETLDEVTCPDF_STATIC::empty_string($nonstroking)) {
		...
		foreach ($bars as $col) {
			switch ($col) {
				...
				default: { // SPECIFIC SPOT COLOR NAME
					$col_a = array(0,0,0,0,'None');
					$col_b = ETLDEVTCPDF_COLORS::getSpotColor($col, $this->spot_colors);
					if ($col_b === false) {
	...
	public function ImageEps($file, $x='', $y='', $w=0, $h=0, $link='', $useBoundingBox=true, $align='', $palign='', $border=0, $fitonpage=false, $fixoutvals=false) {
		...
		if ($file[0] === '@') { // image from string
			...
			$data = ETLDEVTCPDF_STATIC::fileGetContents($file);
		}
		if ($data === FALSE) {
	 ...
	 * @param $type (string) type of barcode (see etldevtcpdf_barcodes_1d.php for supported formats).
	...
	public function write1DBarcode($code, $type, $x='', $y='', $w='', $h='', $xres='', $style=array(), $align='') {
		if (ETLDEVTCPDF_STATIC::empty_string(trim($code))) {
			return;
		}
		require_once(dirname(__FILE__).'/etldevtcpdf_barcodes_1d.php');
		// save current graphic settings
		$gvars = $this->getGraphicVars();
		...
		if ($style['stretch']) {
			...
			if (ETLDEVTCPDF_STATIC::empty_string($xres)) {
		...
		// print text
		if ($style['text']) {
			if (isset($style['label']) AND !ETLDEVTCPDF_STATIC::empty_string($style['label'])) {
				$label = $style['label'];
			} else {
				$label = $code;
			}
	 ...
	 * Print 2D Barcode.
	 * @param $code (string) code to print
	 * @param $type (string) type of barcode (see etldevtcpdf_barcodes_2d.php for supported formats).
	...
	public function write2DBarcode($code, $type, $x='', $y='', $w='', $h='', $style=array(), $align='', $distort=false) {
		...
		require_once(dirname(__FILE__).'/etldevtcpdf_barcodes_2d.php');
		// save current graphic settings
		$gvars = $this->getGraphicVars();
		// create new barcode object
	...
	public function fixHTMLCode($html, $default_css='', $tagvs='', $tidy_options='') {
		return ETLDEVTCPDF_STATIC::fixHTMLCode($html, $default_css, $tagvs, $tidy_options, $this->tagvspaces);
	}
	...
	protected function getCSSBorderStyle($cssborder) {
		...
		$border['color'] = ETLDEVTCPDF_COLORS::convertHTMLColorToDec($color, $this->spot_colors);
		return $border;
	}
	...
	public function getHTMLFontUnits($val, $refsize=12, $parent_size=12, $defaultunit='pt') {
		$refsize = ETLDEVTCPDF_FONTS::getFontRefSize($refsize);
		$parent_size = ETLDEVTCPDF_FONTS::getFontRefSize($parent_size, $refsize);
		switch ($val) {
			case 'xx-small': {
				$size = ($refsize - 4);
				break;
	...
	protected function getHtmlDomArray($html) {
				...
				if (preg_match('/type[\s]*=[\s]*"text\/css"/', $link, $type)) {
					...
					if (empty($type) OR (isset($type[1]) AND (($type[1] == 'all') OR ($type[1] == 'print')))) {
						...
						if (preg_match('/href[\s]*=[\s]*"([^"]*)"/', $link, $type) > 0) {
							// read CSS data file
							$cssdata = ETLDEVTCPDF_STATIC::fileGetContents(trim($type[1]));
							if (($cssdata !== FALSE) AND (strlen($cssdata) > 0)) {
								$css = array_merge($css, ETLDEVTCPDF_STATIC::extractCSSproperties($cssdata));
							}
						}
					}
				}
		...
		// define block tags
		$blocktags = array('blockquote','br','dd','dl','div','dt','h1','h2','h3','h4','h5','h6','hr','li','ol','p','pre','ul','etldevtcpdf','table','tr','td');
		// define self-closing tags
		$selfclosingtags = array('area','base','basefont','br','hr','input','img','link','meta');
		// remove all unsupported tags (the line below lists all supported tags)
		$html = strip_tags($html, '<marker/><a><b><blockquote><body><br><br/><dd><del><div><dl><dt><em><font><form><h1><h2><h3><h4><h5><h6><hr><hr/><i><img><input><label><li><ol><option><p><pre><s><select><small><span><strike><strong><sub><sup><table><tablehead><etldevtcpdf><td><textarea><th><thead><tr><tt><u><ul>');
		//replace some blank characters
		$html = preg_replace('/<pre/', '<xre', $html); // preserve pre tag
		$html = preg_replace('/<(table|tr|td|th|etldevtcpdf|blockquote|dd|div|dl|dt|form|h1|h2|h3|h4|h5|h6|br|hr|li|ol|ul|p)([^\>]*)>[\n\r\t]+/', '<\\1\\2>', $html);
		$html = preg_replace('@(\r\n|\r)@', "\n", $html);
			...
			if (preg_match($tagpattern, $element)) {
				...
				if ($element[0] == '/') {
					...
					if (($dom[$key]['value'] == 'tr') AND ($dom[($dom[$key]['parent'])]['thead'] === true)) {
						if (ETLDEVTCPDF_STATIC::empty_string($dom[($dom[($dom[$key]['parent'])]['parent'])]['thead'])) {
					...
					if (($dom[$key]['value'] == 'table') AND (!ETLDEVTCPDF_STATIC::empty_string($dom[($dom[$key]['parent'])]['thead']))) {
					...
					if (!empty($css)) {
						...
						list($dom[$key]['csssel'], $dom[$key]['cssdata']) = ETLDEVTCPDF_STATIC::getCSSdataArray($dom, $key, $css);
						$dom[$key]['attribute']['style'] = ETLDEVTCPDF_STATIC::getTagStyleFromCSSarray($dom[$key]['cssdata']);
					}
					...
					if (isset($dom[$key]['attribute']['style']) AND !empty($dom[$key]['attribute']['style'])) {
						...
						// font color
						if (isset($dom[$key]['style']['color']) AND (!ETLDEVTCPDF_STATIC::empty_string($dom[$key]['style']['color']))) {
							$dom[$key]['fgcolor'] = ETLDEVTCPDF_COLORS::convertHTMLColorToDec($dom[$key]['style']['color'], $this->spot_colors);
						} elseif ($dom[$key]['value'] == 'a') {
							$dom[$key]['fgcolor'] = $this->htmlLinkColorArray;
						}
						// background color
						if (isset($dom[$key]['style']['background-color']) AND (!ETLDEVTCPDF_STATIC::empty_string($dom[$key]['style']['background-color']))) {
							$dom[$key]['bgcolor'] = ETLDEVTCPDF_COLORS::convertHTMLColorToDec($dom[$key]['style']['background-color'], $this->spot_colors);
						}
						// text-decoration
						if (isset($dom[$key]['style']['text-decoration'])) {
							...
							foreach ($decors as $dec) {
								$dec = trim($dec);
								if (!ETLDEVTCPDF_STATIC::empty_string($dec)) {
									if ($dec[0] == 'u') {
										// underline
										$dom[$key]['fontstyle'] .= 'U';
									} elseif ($dec[0] == 'l') {
						...
						if (isset($dom[$key]['style']['border-color'])) {
							...
							if (isset($brd_colors[3])) {
								$dom[$key]['border']['L']['color'] = ETLDEVTCPDF_COLORS::convertHTMLColorToDec($brd_colors[3], $this->spot_colors);
							}
							if (isset($brd_colors[1])) {
								$dom[$key]['border']['R']['color'] = ETLDEVTCPDF_COLORS::convertHTMLColorToDec($brd_colors[1], $this->spot_colors);
							}
							if (isset($brd_colors[0])) {
								$dom[$key]['border']['T']['color'] = ETLDEVTCPDF_COLORS::convertHTMLColorToDec($brd_colors[0], $this->spot_colors);
							}
							if (isset($brd_colors[2])) {
								$dom[$key]['border']['B']['color'] = ETLDEVTCPDF_COLORS::convertHTMLColorToDec($brd_colors[2], $this->spot_colors);
							}
						}
						...
						foreach ($cellside as $bsk => $bsv) {
							...
							if (isset($dom[$key]['style']['border-'.$bsv.'-color'])) {
								$dom[$key]['border'][$bsk]['color'] = ETLDEVTCPDF_COLORS::convertHTMLColorToDec($dom[$key]['style']['border-'.$bsv.'-color'], $this->spot_colors);
							}
					...
					if ((($dom[$key]['value'] == 'ul') OR ($dom[$key]['value'] == 'ol') OR ($dom[$key]['value'] == 'dl'))
						AND (!isset($dom[$key]['align']) OR ETLDEVTCPDF_STATIC::empty_string($dom[$key]['align']) OR ($dom[$key]['align'] != 'J'))) {
						if ($this->rtl) {
							$dom[$key]['align'] = 'R';
						} else {
							$dom[$key]['align'] = 'L';
					...
					if (isset($dom[$key]['attribute']['color']) AND (!ETLDEVTCPDF_STATIC::empty_string($dom[$key]['attribute']['color']))) {
						$dom[$key]['fgcolor'] = ETLDEVTCPDF_COLORS::convertHTMLColorToDec($dom[$key]['attribute']['color'], $this->spot_colors);
					} elseif (!isset($dom[$key]['style']['color']) AND ($dom[$key]['value'] == 'a')) {
						$dom[$key]['fgcolor'] = $this->htmlLinkColorArray;
					}
					// set background color attribute
					if (isset($dom[$key]['attribute']['bgcolor']) AND (!ETLDEVTCPDF_STATIC::empty_string($dom[$key]['attribute']['bgcolor']))) {
						$dom[$key]['bgcolor'] = ETLDEVTCPDF_COLORS::convertHTMLColorToDec($dom[$key]['attribute']['bgcolor'], $this->spot_colors);
					}
					// set stroke color attribute
					if (isset($dom[$key]['attribute']['strokecolor']) AND (!ETLDEVTCPDF_STATIC::empty_string($dom[$key]['attribute']['strokecolor']))) {
						$dom[$key]['strokecolor'] = ETLDEVTCPDF_COLORS::convertHTMLColorToDec($dom[$key]['attribute']['strokecolor'], $this->spot_colors);
					}
					// check for width attribute
					...
					// check for text alignment
					if (isset($dom[$key]['attribute']['align']) AND (!ETLDEVTCPDF_STATIC::empty_string($dom[$key]['attribute']['align'])) AND ($dom[$key]['value'] !== 'img')) {
	...
	/**
	 * Return an hash code used to ensure that the serialized data has been generated by this ETLDEVTCPDF instance.
	...
	/**
	 * Serialize an array of parameters to be used with ETLDEVTCPDF tag in HTML code.
	...
	/**
	 * Unserialize parameters to be used with ETLDEVTCPDF tag in HTML code.
	 * @param $data (string) serialized data
	 ...
	 * Supported tags are: a, b, blockquote, br, dd, del, div, dl, dt, em, font, h1, h2, h3, h4, h5, h6, hr, i, img, li, ol, p, pre, small, span, strong, sub, sup, table, etldevtcpdf, td, th, thead, tr, tt, u, ul
...
Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
	...
	public function writeHTML($html, $ln=true, $fill=false, $reseth=false, $cell=false, $align='') {
		...
		while ($key < $maxel) {
			...
			if (($dom[$key]['value'] == 'tr') AND isset($dom[$key]['thead']) AND $dom[$key]['thead']) {
				if (isset($dom[$key]['parent']) AND isset($dom[$dom[$key]['parent']]['thead']) AND !ETLDEVTCPDF_STATIC::empty_string($dom[$dom[$key]['parent']]['thead'])) {
					$this->inthead = true;
			...
			if ($dom[$key]['tag'] OR ($key == 0)) {
				...
				if (ETLDEVTCPDF_STATIC::empty_string($lalign)) {
					$lalign = $align;
				}
			}
			// align lines
				...
				if (isset($startlinex)) {
					...
					if ((isset($plalign) AND ((($plalign == 'C') OR ($plalign == 'J') OR (($plalign == 'R') AND (!$this->rtl)) OR (($plalign == 'L') AND ($this->rtl)))))) {
						...
						if ($this->isRTLTextDir()) { // RTL
							...
							$pos1 = ETLDEVTCPDF_STATIC::revstrpos($pmid, '[(');
							if ($pos1 > 0) {
								$pos1 = intval($pos1);
								if ($this->isUnicodeFont()) {
									$pos2 = intval(ETLDEVTCPDF_STATIC::revstrpos($pmid, '[('.chr(0).chr(32)));
									$spacelen = 2;
								} else {
									$pos2 = intval(ETLDEVTCPDF_STATIC::revstrpos($pmid, '[('.chr(32)));
									$spacelen = 1;
								}
								if ($pos1 == $pos2) {
							...
							if ($pos1 > 0) {
								$pos1 = intval($pos1);
								if ($this->isUnicodeFont()) {
									$pos2 = intval(ETLDEVTCPDF_STATIC::revstrpos($pmid, chr(0).chr(32).')]')) + 2;
									$spacelen = 2;
								} else {
									$pos2 = intval(ETLDEVTCPDF_STATIC::revstrpos($pmid, chr(32).')]')) + 1;
									$spacelen = 1;
								}
								if ($pos1 == $pos2) {
							...
							if (preg_match_all('/\[\(([^\)]*)\)\]/x', $pmidtemp, $lnstring, PREG_PATTERN_ORDER)) {
								...
								while (preg_match('/([0-9\.\+\-]*)[\s](Td|cm|m|l|c|re)[\s]/x', $pmid, $strpiece, PREG_OFFSET_CAPTURE, $offset) == 1) {
									...
									switch ($strpiece[2][0]) {
										...
										case 're': {
											// justify block
											if (!ETLDEVTCPDF_STATIC::empty_string($this->lispacer)) {
												$this->lispacer = '';
												break;
											}
			...
			if ($dom[$key]['tag']) {
				...
				// print list-item
				if (!ETLDEVTCPDF_STATIC::empty_string($this->lispacer) AND ($this->lispacer != '^')) {
				...
				// text
				$this->htmlvspace = 0;
				$isRTLString = preg_match(ETLDEVTCPDF_FONT_DATA::$uni_RE_PATTERN_RTL, $dom[$key]['value']) || preg_match(ETLDEVTCPDF_FONT_DATA::$uni_RE_PATTERN_ARABIC, $dom[$key]['value']);
				...
				if (!isset($dom[$key]['trimmed_space']) OR !$dom[$key]['trimmed_space']) {
					...
					if (!empty($this->HREF) AND (isset($this->HREF['url']))) {
						...
						if (($strlinelen < $cwa) AND (isset($dom[($key + 1)])) AND ($dom[($key + 1)]['tag']) AND (!$dom[($key + 1)]['block'])) {
							...
							while ($write_block AND isset($dom[$nkey])) {
								if ($dom[$nkey]['tag']) {
									...
									$nextstr = ETLDEVTCPDF_STATIC::pregSplit('/'.$this->re_space['p'].'+/', $this->re_space['m'], $dom[$nkey]['value']);
						...
						if (($wadj > 0) AND (($strlinelen + $wadj) >= $cwa)) {
							$wadj = 0;
							$nextstr = ETLDEVTCPDF_STATIC::pregSplit('/'.$this->re_space['p'].'/', $this->re_space['m'], $dom[$key]['value']);
	...
	protected function openHTMLTagHandler($dom, $key, $cell) {
		...
		switch($tag['value']) {
			case 'table': {
				...
				if (!isset($dom[$key]['attribute']['nested']) OR ($dom[$key]['attribute']['nested'] != 'true')) {
					$this->htmlvspace = 0;
					// set table header
					if (!ETLDEVTCPDF_STATIC::empty_string($dom[$key]['thead'])) {
						// set table header
			...
			case 'img': {
				...
				if ($imgsrc[0] === '@') {
					...
					// get image type
					$type = ETLDEVTCPDF_IMAGES::getImageFileType($imgsrc);
				}
				if (!isset($tag['width'])) {
					$tag['width'] = 0;
				}
				...
				$xpos = $this->x;
				$imglink = '';
				if (isset($this->HREF['url']) AND !ETLDEVTCPDF_STATIC::empty_string($this->HREF['url'])) {
			...
			case 'li': {
				...
				if ($this->listordered[$this->listnum]) {
					// ordered item
					if (isset($parent['attribute']['type']) AND !ETLDEVTCPDF_STATIC::empty_string($parent['attribute']['type'])) {
						$this->lispacer = $parent['attribute']['type'];
					} elseif (isset($parent['listtype']) AND !ETLDEVTCPDF_STATIC::empty_string($parent['listtype'])) {
						$this->lispacer = $parent['listtype'];
					} elseif (isset($this->lisymbol) AND !ETLDEVTCPDF_STATIC::empty_string($this->lisymbol)) {
			...
			case 'input': {
				if (isset($tag['attribute']['name']) AND !ETLDEVTCPDF_STATIC::empty_string($tag['attribute']['name'])) {
				...
				$prop = array();
				$opt = array();
				if (isset($tag['attribute']['readonly']) AND !ETLDEVTCPDF_STATIC::empty_string($tag['attribute']['readonly'])) {
					$prop['readonly'] = true;
				}
				if (isset($tag['attribute']['value']) AND !ETLDEVTCPDF_STATIC::empty_string($tag['attribute']['value'])) {
					$value = $tag['attribute']['value'];
				}
				if (isset($tag['attribute']['maxlength']) AND !ETLDEVTCPDF_STATIC::empty_string($tag['attribute']['maxlength'])) {
					$opt['maxlen'] = intval($tag['attribute']['maxlength']);
				}
				$h = $this->getCellHeight($this->FontSize);
				if (isset($tag['attribute']['size']) AND !ETLDEVTCPDF_STATIC::empty_string($tag['attribute']['size'])) {
				...
				switch ($tag['attribute']['type']) {
					...
					case 'image': {
						...
						if (isset($tag['attribute']['src']) AND !ETLDEVTCPDF_STATIC::empty_string($tag['attribute']['src'])) {
			...
			case 'textarea': {
				...
				if (isset($tag['attribute']['cols']) AND !ETLDEVTCPDF_STATIC::empty_string($tag['attribute']['cols'])) {
					$w = intval($tag['attribute']['cols']) * $this->GetStringWidth(chr(32)) * 2;
				} else {
					$w = 40;
				}
				if (isset($tag['attribute']['rows']) AND !ETLDEVTCPDF_STATIC::empty_string($tag['attribute']['rows'])) {
			...
			case 'select': {
				...
				$w = 0;
				if (isset($tag['attribute']['opt']) AND !ETLDEVTCPDF_STATIC::empty_string($tag['attribute']['opt'])) {
			...
			case 'etldevtcpdf': {
				if (defined('K_ETLDEVTCPDF_CALLS_IN_HTML') AND (K_ETLDEVTCPDF_CALLS_IN_HTML === true)) {
					// Special tag used to call ETLDEVTCPDF methods
					if (isset($tag['attribute']['method'])) {
						$etldevtcpdf_method = $tag['attribute']['method'];
						if (method_exists($this, $etldevtcpdf_method)) {
							if (isset($tag['attribute']['params']) AND (!empty($tag['attribute']['params']))) {
								$params = $this->unserializeTCPDFtagParameters($tag['attribute']['params']);
								call_user_func_array(array($this, $etldevtcpdf_method), $params);
							} else {
								$this->$etldevtcpdf_method();
							}
							$this->newline = true;
						}
					}
		...
		$bordertags = array('blockquote','br','dd','dl','div','dt','h1','h2','h3','h4','h5','h6','hr','li','ol','p','pre','ul','etldevtcpdf','table');
	...
	protected function drawHTMLTagBorder($tag, $xmax) {
		...
		if (isset($tag['border']) AND !empty($tag['border'])) {
			...
			if (!ETLDEVTCPDF_STATIC::empty_string($this->thead) AND (!$this->inthead)) {
				...
				$border = ETLDEVTCPDF_STATIC::getBorderMode($border, $position='middle', $this->opencell);
			}
		}
	...
	protected function putHtmlListBullet($listdepth, $listtype='', $size=10) {
		...
		switch ($listtype) {
			...
			case 'i':
			case 'lower-roman': {
				$textitem = strtolower(ETLDEVTCPDF_STATIC::intToRoman($this->listcount[$this->listnum]));
				break;
			}
			case 'I':
			case 'upper-roman': {
				$textitem = ETLDEVTCPDF_STATIC::intToRoman($this->listcount[$this->listnum]);
				break;
			}
			case 'a':
			case 'lower-alpha':
			...
			case 'lower-greek': {
				$textitem = ETLDEVTCPDF_FONTS::unichr((945 + $this->listcount[$this->listnum] - 1), $this->isunicode);
				break;
			}
			/*
		...
		if (!ETLDEVTCPDF_STATIC::empty_string($textitem)) {
	...
	public function addTOC($page='', $numbersfont='', $filler='.', $toc_name='TOC', $style='', $color=array(0,0,0)) {
		...
		$page_fill_end = false;
		$current_column = $this->current_column;
		if (ETLDEVTCPDF_STATIC::empty_string($numbersfont)) {
			$numbersfont = $this->default_monospaced_font;
		}
		if (ETLDEVTCPDF_STATIC::empty_string($filler)) {
			$filler = ' ';
		}
		if (ETLDEVTCPDF_STATIC::empty_string($page)) {
			$gap = ' ';
		} else {
			$gap = '';
			if ($page < 1) {
		...
		if (!ETLDEVTCPDF_STATIC::empty_string($page)) {
			for ($p = $page_first; $p <= $page_last; ++$p) {
				...
				for ($n = 1; $n <= $maxpage; ++$n) {
					...
					$na = ETLDEVTCPDF_STATIC::formatTOCPageNumber(($this->starting_page_number + $np - 1));
					$nu = ETLDEVTCPDF_FONTS::UTF8ToUTF16BE($na, false, $this->isunicode, $this->CurrentFont);
					// replace aliases with numbers
					foreach ($pnalias['u'] as $u) {
						$sfill = str_repeat($filler, max(0, (strlen($u) - strlen($nu.' '))));
						if ($this->rtl) {
							$nr = $nu.ETLDEVTCPDF_FONTS::UTF8ToUTF16BE(' '.$sfill, false, $this->isunicode, $this->CurrentFont);
						} else {
							$nr = ETLDEVTCPDF_FONTS::UTF8ToUTF16BE($sfill.' ', false, $this->isunicode, $this->CurrentFont).$nu;
						}
	...
	public function addHTMLTOC($page='', $toc_name='TOC', $templates=array(), $correct_align=true, $style='', $color=array(0,0,0)) {
		...
		foreach ($this->outlines as $key => $outline) {
			...
			$row = str_replace('#TOC_INDEX#', $bookmark_index, $row);
	...
	/**
	 * Stores a copy of the current ETLDEVTCPDF object used for undo operation.
	 * @public
	 * @since 4.5.029 (2009-03-19)
	 */
	public function startTransaction() {
		...
		$this->objcopy = ETLDEVTCPDF_STATIC::objclone($this);
	}

	/**
	 * Delete the copy of the current ETLDEVTCPDF object used for undo operation.
	 * @public
	 * @since 4.5.029 (2009-03-19)
	 */
	...
	/**
	 * This method allows to undo the latest transaction by returning the latest saved ETLDEVTCPDF object with startTransaction().
	...
	*/
	...
	protected function hyphenateWord($word, $patterns, $dictionary=array(), $leftmin=1, $rightmin=2, $charmin=1, $charmax=8) {
		...
		$word_string = ETLDEVTCPDF_FONTS::UTF8ArrSubString($word, '', '', $this->isunicode);
		...
		if (isset($dictionary[$word_string])) {
			return ETLDEVTCPDF_FONTS::UTF8StringToArray($dictionary[$word_string], $this->isunicode, $this->CurrentFont);
		}
		...
		for ($pos = 0; $pos < $maxpos; ++$pos) {
			...
			for ($i = 1; $i <= $imax; ++$i) {
				$subword = strtolower(ETLDEVTCPDF_FONTS::UTF8ArrSubString($tmpword, $pos, ($pos + $i), $this->isunicode));
				if (isset($patterns[$subword])) {
					$pattern = ETLDEVTCPDF_FONTS::UTF8StringToArray($patterns[$subword], $this->isunicode, $this->CurrentFont);
	...
	public function hyphenateText($text, $patterns, $dictionary=array(), $leftmin=1, $rightmin=2, $charmin=1, $charmax=8) {
		...
		if (!is_array($patterns)) {
			$patterns = ETLDEVTCPDF_STATIC::getHyphenPatternsFromTEX($patterns);
		}
		// get array of characters
		$unichars = ETLDEVTCPDF_FONTS::UTF8StringToArray($text, $this->isunicode, $this->CurrentFont);
		// for each char
		foreach ($unichars as $char) {
			if ((!$intag) AND (!$skip) AND ETLDEVTCPDF_FONT_DATA::$uni_type[$char] == 'L') {
				// letter character
				$word[] = $char;
			} else {
				// other type of character
				if (!ETLDEVTCPDF_STATIC::empty_string($word)) {
					// hypenate the word
		...
		return ETLDEVTCPDF_FONTS::UTF8ArrSubString($txtarr, '', '', $this->isunicode);
	}
	...
	public function printTemplate($id, $x='', $y='', $w=0, $h=0, $align='', $palign='', $fitonpage=false) {
		...
		if (!empty($this->xobjects[$id]['annotations'])) {
			foreach ($this->xobjects[$id]['annotations'] as $annot) {
				...
				$coordlt = ETLDEVTCPDF_STATIC::getTransformationMatrixProduct($tm, array(1, 0, 0, 1, ($annot['x'] * $this->k), (-$annot['y'] * $this->k)));
				$ax = ($coordlt[4] / $this->k);
				$ay = ($this->h - $h - ($coordlt[5] / $this->k));
				$coordrb = ETLDEVTCPDF_STATIC::getTransformationMatrixProduct($tm, array(1, 0, 0, 1, (($annot['x'] + $annot['w']) * $this->k), ((-$annot['y'] - $annot['h']) * $this->k)));
	...
	public function ImageSVG($file, $x='', $y='', $w=0, $h=0, $link='', $align='', $palign='', $border=0, $fitonpage=false) {
		...
		if ($file[0] === '@') { // image from string
			...
			$svgdata = ETLDEVTCPDF_STATIC::fileGetContents($file);
		}
		if ($svgdata === FALSE) {
	...
	protected function setSVGStyles($svgstyle, $prevsvgstyle, $x=0, $y=0, $w=1, $h=1, $clip_function='', $clip_params=array()) {
		...
		// color
		$fill_color = ETLDEVTCPDF_COLORS::convertHTMLColorToDec($svgstyle['color'], $this->spot_colors);
		$this->SetFillColorArray($fill_color);
		// text color
		$text_color = ETLDEVTCPDF_COLORS::convertHTMLColorToDec($svgstyle['text-color'], $this->spot_colors);
		...
		if (preg_match('/url\([\s]*\#([^\)]*)\)/si', $svgstyle['fill'], $regs)) {
			...
			$fill_color = ETLDEVTCPDF_COLORS::convertHTMLColorToDec($svgstyle['fill'], $this->spot_colors);
		...
		if ($svgstyle['stroke'] != 'none') {
			...
			$stroke_style = array(
				'color' => ETLDEVTCPDF_COLORS::convertHTMLColorToDec($svgstyle['stroke'], $this->spot_colors),
					...
					foreach ($params as $ck => $cp) {
						...
						if ((($ck + 1) % 7) == 0) {
							...
							if ((abs($x0 - $x) < $minlen) AND (abs($y0 - $y) < $minlen)) {
								...
								// get angles
								$angs = ETLDEVTCPDF_STATIC::getVectorsAngle(1, 0, (($xa - $cax) / $rx), (($cay - $ya) / $ry));
								$dang = ETLDEVTCPDF_STATIC::getVectorsAngle((($xa - $cax) / $rx), (($ya - $cay) / $ry), ((-$xa - $cax) / $rx), ((-$ya - $cay) / $ry));
								if (($fs == 0) AND ($dang > 0)) {
									$dang -= (2 * M_PI);
	...
	protected function startSVGElementHandler($parser, $name, $attribs, $ctm=array()) {
		...
		if (isset($attribs['style']) AND !ETLDEVTCPDF_STATIC::empty_string($attribs['style']) AND ($attribs['style'][0] != ';')) {
			// fix style for regular expression
			$attribs['style'] = ';'.$attribs['style'];
		}
		foreach ($prev_svgstyle as $key => $val) {
			if (in_array($key, ETLDEVTCPDF_IMAGES::$svginheritprop)) {
				// inherit previous value
				$svgstyle[$key] = $val;
			}
			if (isset($attribs[$key]) AND !ETLDEVTCPDF_STATIC::empty_string($attribs[$key])) {
			...
			} elseif (isset($attribs['style']) AND !ETLDEVTCPDF_STATIC::empty_string($attribs['style'])) {
				// CSS style syntax
				$attrval = array();
		...
		if (isset($attribs['transform']) AND !empty($attribs['transform'])) {
			$tm = ETLDEVTCPDF_STATIC::getTransformationMatrixProduct($tm, ETLDEVTCPDF_STATIC::getSVGTransformMatrix($attribs['transform']));
		}
		...
		switch($name) {
			...
			case 'svg': {
				...
				$tm = ETLDEVTCPDF_STATIC::getTransformationMatrixProduct($tm, array( 1, 0, 0, 1, $svgX, $svgY));
				...
				if (isset($attribs['viewBox'])) {
					...
					if (sizeof($tmp) == 4) {
						...
						$tm = ETLDEVTCPDF_STATIC::getTransformationMatrixProduct($tm, $newtm);
						$this->SVGTransform($tm);
					}
				}
			...
			case 'g': {
				...
				$tm = ETLDEVTCPDF_STATIC::getTransformationMatrixProduct($tm, array($w, 0, 0, $h, $x, $y));
				$this->SVGTransform($tm);
				$this->setSVGStyles($svgstyle, $prev_svgstyle);
				break;
			}
			case 'linearGradient': {
				...
				if (isset($attribs['gradientTransform'])) {
					$this->svggradients[$this->svggradientid]['gradientTransform'] = ETLDEVTCPDF_STATIC::getSVGTransformMatrix($attribs['gradientTransform']);
				}
			...
			case 'stop': {
				...
				$stop_color = isset($svgstyle['stop-color'])?ETLDEVTCPDF_COLORS::convertHTMLColorToDec($svgstyle['stop-color'], $this->spot_colors):'black';
			...
			case 'image': {
				...
				if (!$clipping) {
					...
					if (preg_match('/^data:image\/[^;]+;base64,/', $img, $m) > 0) {
						...
						// fix image path
						if (!ETLDEVTCPDF_STATIC::empty_string($this->svgdir) AND (($img[0] == '.') OR (basename($img) == $img))) {
					...
					// get image type
					$imgtype = ETLDEVTCPDF_IMAGES::getImageFileType($img);
					if (($imgtype == 'eps') OR ($imgtype == 'ai')) {
